...
async function pushPending() {
  await ElMessageBox.confirm(`确认处理待推送通知? 本次最多 ${pushForm.limit} 条`, '通知推送', { type: 'warning' })
  actionLoading.value = true
  try {
    const result = await pushPendingNotifications(Number(pushForm.limit || 20))
    ElMessage.success('待推送处理完成')
    lastAction.value = `待推送处理完成: ${JSON.stringify(result)}，${new Date().toLocaleString()}`
    await loadConsumer()
  } finally {
  ...
  <div>
    <el-row :gutter="16" class="app-card">
      <el-col :xs="24" :sm="12" :md="6">
        <el-card shadow="never" class="stat-card">
          <div class="table-toolbar"><span>服务</span><el-icon color="#409eff" :size="24"><Bell /></el-icon></div>
    ...
    <el-card shadow="never">
      ...
      <el-tabs v-model="activeTab">
        ...
        <el-tab-pane label="待推送处理" name="push">
          <el-form :inline="true" :model="pushForm" class="filter-form">
            <el-form-item label="条数"><el-input-number v-model="pushForm.limit" :min="1" :max="200" /></el-form-item>
            <el-form-item>
              <el-button type="primary" :loading="actionLoading" @click="pushPending">处理待推送</el-button>
            </el-form-item>
          </el-form>
          <pre class="json-preview">{{ JSON.stringify({ lastAction, summary }, null, 2) }}</pre>
        </el-tab-pane>
