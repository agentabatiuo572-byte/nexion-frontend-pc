...
function backendPath(parts: string[]) {
  if (parts.length === 1 && parts[0] === "docs") return "/api/admin/developer/docs";
  if (!parts.length || parts[0] !== "access-requests") return null;
  if (parts.length > 3 || parts.some((part) => !part.trim() || part.includes("..") || part.includes("/") || part.includes("\\"))) {
    return null;
  }
  return `/api/admin/developer/${parts.map((part) => encodeURIComponent(part)).join("/")}`;
}

async function proxy(request: Request, context: RouteContext) {
  const { path = [] } = await context.params;
  const targetPath = backendPath(path);
  if (!targetPath) return jsonError(404, "DEVELOPER_ACCESS_ROUTE_NOT_FOUND");
  ...
  try {
    ...
    return jsonError(503, "DEVELOPER_ACCESS_BACKEND_UNAVAILABLE");
  }
}
