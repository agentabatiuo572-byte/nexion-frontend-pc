 ...
 * 处置(补发通知/清理/改偏好)= confirm→MC(改他人偏好属高敏)。CGM: A-001/003/004/005。
 */
...
export function NotificationSection({ user }: { user: AdminUser }) {
  ...
  async function doSend() {
    const yes = await confirm({ title: "向该用户补发通知?", message: `向 ${user.nickname} 推送一条运营通知。需第二角色复核 + 审计。`, confirmLabel: "补发" });
    if (yes) {
      opsLog(user.id, "补发通知", `向 ${user.nickname} 推送运营通知`, "neutral");
      toast.success("通知已补发", `${user.id} · ${user.nickname}`);
    }
  }
  ...
  return (
    <HubCard icon={<Bell size={15} style={{ color: "var(--admin-domain-i)" }} />} title="通知·偏好卡" tag="C1·deepening · A/I · 改偏好 MC">
      ...
      <p className="mt-2 flex flex-wrap items-center gap-2 text-[10.5px]" style={{ color: "var(--v5-ink-4)" }}>
        ...
        <AutoGloss>补发/清理/改订阅偏好在 I 域 · 改他人偏好 MC 双签。</AutoGloss>
      </p>
    </HubCard>
  );
}
