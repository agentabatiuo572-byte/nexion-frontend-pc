...
export function J1KillSwitch({ ctx }: { ctx: JCtx }) {
  ...
  const killGate = (g: Gate, emer: boolean) => openActionConfirm({
    action: emer ? `应急熔断 · ${g.key} · emergency=true` : `Kill-Switch 熔断 · ${g.key}`,
    detail: emer ? (
      <><b>应急快速通道</b>:{g.key} 熔断提案进 A2 队列最高优先级 · 执行门槛响应时限压至 <b>{slaMins} 分钟</b> · 超时自动逐级升级呼叫(I3 critical 通道)· A2 记录标 <b>emergency=true</b> 高亮审计 · 仅止血方向适用 · 不取消确认理由(§15.1)。</>
    ) : (
      <><b>{g.key}</b>({g.cap})· {g.desc} · 资金语义:<b>{IMPACT_LABEL[g.coverageImpactCategory]}</b> · 熔断方向不前置 B1 · 常规轨(emergency=false)· server 即时拒绝下游能力请求。<b>处置预案(disposition_plan,可选)</b>:在途请求冻结待恢复 · 客服话术同步 · 恢复条件 = 根因消除 + 执行门槛操作确认。</>
    ),
    run: (reason) => {
      ...
      toast((emer ? "应急熔断 " : "已熔断 ") + g.key + (emer ? " · A2 emergency=true" : " · 写 A2"));
    },
  });

  const resumeGate = (g: Gate) => openActionConfirm({
    action: `Kill-Switch 恢复 · ${g.key}`,
    amplifies: g.amplifies,
    detail: (
      <><b>{g.key}</b> 从已关停 → 在线 · {g.cap} · {g.coveragePrecheckRequired
  ...
  const launchBatch = () => {
    ...
    openActionConfirm({
      ...
      detail: (
        <>一次性熔断多个功能闸 · 用于<b>监管点名 / 法务事件</b>等重大合规触发 · 工单进 A2 队列最高优先级 · 执行门槛 SLA <b>{slaMins} 分钟</b> · 所有步骤标 emergency=true 高亮审计 · 默认勾选「立即出钱」闸:<b>{targets.map((g) => g.key).join(" / ") || "—(均已熔断)"}</b> · 每闸独立写 A2 事件。</>
      ),
      run: (reason) => {
        targets.forEach((g) => {
      ...
      <div className="gates-strip">
        {KILLSWITCH.map((g) => { const on = effOn(g); return (
          <div key={g.key} className={"gate-card" + (on ? "" : " killed")}>
            <div className="top"><span className="key">{g.key}</span><span className="led" /></div>
