...
export function J2GeoBlock({ ctx }: { ctx: JCtx }) {
  ...
  const rmBanned = (c: Entry) => openActionConfirm({
    ...
    detail: <>从 activeCountries 移除 <b>{c.cc}</b>({c.name})· <b>恢复方向</b>:该国 IP 段重新开放对应功能入口 · 已存量账户从只读态切回完整状态 · 同步重算 B 域漏斗地域归因 · <b>合规审计 执行门槛 必参</b>(非财务)· 解封恒走常规轨。</>,
    run: (reason) => { setParam(`J.geo.${c.cc}`, "allowed", { action: `黑名单解封 ${c.cc}(activeCountries 移除)`, reason }); toast(`${c.cc} 已解封 · 已移出 activeCountries`); },
  });
  ...
  const addTo = (list: "banned" | "limited") => openActionConfirm({
    action: list === "banned" ? "新增黑名单 · activeCountries" : "新增受限名单",
    detail: list === "banned"
      ? <><b>加入新国家到黑名单</b>(输入 ISO 3166-1 两位国家码)· 走风控 操作员 · 执行门槛:合规审计 (财务不参与)· 加封锁方向<b>可走应急快速通道</b>(监管点名场景)· A2 写 active_countries(before→after)。</>
  ...
  const editEndpoint = (e: (typeof GEO_ENDPOINTS)[number]) => {
    ...
    openActionConfirm({
      action: `编辑 endpoint geo_block · ${e.ep}`,
      detail: <><b>{e.ep}</b>({e.domain})· 当前屏蔽:<b className="mono">[{cur || "未设置"}]</b> · 设置方式:<b>{GEO_SRC_LABEL[e.src]}</b> · {e.srcDesc} · 编辑后该入口的屏蔽国家独立生效 · 不影响全局黑名单 · {e.src === "explicit" ? "genesis 唯一前端显式声明 geo_block(§9.11d.1)。" : e.src === "pending" ? "设置方式待补(V4 落地收口)。" : "默认继承全局黑名单 · 编辑后转为单独设定。"}</>,
      edit: { kind: "text", current: cur || "—" },
      run: (reason, newValue) => {
        setParam(`J.geo.endpoint.${e.ep}`, (newValue ?? "").toUpperCase(), { action: `编辑 endpoint geo_block ${e.ep}`, reason });
        toast(`${e.ep} 屏蔽国家已确认生效`);
      },
    });
  };
  const switchJudge = () => {
    const cur = pget("J.geo.edgeJudgeSource") ?? "server-edge-ip";
    openActionConfirm({
      action: "切换边缘 IP 判定源",
  ...
  const emergencyBlock = () => openActionConfirm({
    action: "应急即时封锁 · activeCountries 批量加入",
      ...
      <div className="geo-grid">
        <section className="geo-card">
          ...
          <div className="country-list">
            {banned.length === 0 && <span style={{ color: "var(--ink-4)", fontSize: 11.5, padding: "8px 0" }}>活跃黑名单为空 · enabled = false</span>}
            {banned.map((c) => (
          ...
          {GEO_ENDPOINTS.map((e) => {
            ...
            return (
              <div className="rw" key={e.ep}>
                <div className="c endpoint">{e.ep}</div>
                <div className="c"><span className="domain">{e.domain}</span></div>
      ...
      <div className="geo-grid">
        ...
        <section className="edge-card">
          ...
          <div className="edge-body">
            <div className="edge-kv"><span className="k">判定源</span><span className="v" style={{ display: "flex", alignItems: "center", gap: 8 }}>{pget("J.geo.edgeJudgeSource") ?? "server 边缘 IP"}<button onClick={switchJudge}>切换</button></span></div>
            {GEO_EDGE.map((row) => (
