...
function controlValue(overview: G3Overview, key: string, fallback: string) {
  return overview.controls.find((control) => control.key === key)?.value || fallback;
}
...
export function G3Market({ ctx }: { ctx: GCtx }) {
  ...
  const chartPrices = useMemo(() => {
    ...
    if (!overview) return [0.171, 0.171];
    const framePrices = overview.frames.map((frame) => frame.targetPrice).filter((value) => value > 0);
    return framePrices.length >= 2 ? framePrices : [overview.currentPrice, overview.currentPrice];
  }, [history, overview]);
  ...
  const schedV = controlValue(overview, "schedule", "每日 00:00 UTC 自动推进");
  const scheduleMeta = scheduleControl?.cronExpression
    ? `${scheduleControl.cronExpression} · ${scheduleControl.zone || "UTC"}`
    : "动态 server cron";
  const pinV = controlValue(overview, "pin", "未钉住");
  const loopV = controlValue(overview, "loop", "循环");
  ...
  const H = 180;
  const P = 30;
  const chart = chartPrices.length >= 2 ? chartPrices : [overview.currentPrice, overview.currentPrice];
  const minPrice = Math.min(...chart, overview.currentPrice) * 0.98;
  const maxPrice = Math.max(...chart, peak, overview.currentPrice) * 1.02;
  ...
  const path = chart.map((value, index) => `${index ? "L" : "M"}${X(index).toFixed(1)} ${Y(value).toFixed(1)}`).join(" ");
  const peakY = Y(peak);
  const change = changePct(chart);

  return (
    <>
      {error && <div className="gtint" style={{ marginBottom: 12 }}>G3 操作提示 · {error}</div>}
      <div className="f-stats">
        <div className="f-stat ok"><div className="k">NEX 现价</div><div className="v">{price}</div><div className="sub">24h {change >= 0 ? "+" : ""}{change.toFixed(2)}% · 当日帧 D{curDay} 派发</div></div>
      ...
      <div className="two-col r13" style={{ marginBottom: 16 }}>
        <section className="l-card">
          ...
          <div className="l-b">
            <div className="price-hero"><span className="big">{price}</span><span className="chg">{change >= 0 ? "+" : ""}{change.toFixed(2)}%</span>{paused && <span className="bdg bad">已冻结</span>}</div>
