...
export default function G4AdminOperations({ ctx }: { ctx: GCtx }) {
  ...
  return <>
    ...
    <section className="l-card" style={{ marginBottom: 16 }}>
      <div className="l-h"><span className="ttl">资格门与预售</span><span className="sub">· 服务端配置 · 不注入 mock 数据</span></div>
      <div className="l-b"><div className="param-grid">{CONFIGS.map((definition) => <div className="p" key={definition.key}><div className="k">{definition.label}</div><div className="v">{data.config[definition.key] ?? "未配置"}{canWrite && <button className="l-btn sm mc" onClick={() => edit(definition)}>调整</button>}</div><div className="s">{definition.key}</div></div>)}</div></div>
    </section>
    <section className="l-card" style={{ marginBottom: 16 }}>
