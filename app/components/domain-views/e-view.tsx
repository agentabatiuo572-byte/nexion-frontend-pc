...
export function EDomainView({ meta }: { meta: DomainViewMeta }) {
  ...
  const phaseCur = e1Gates?.phaseCurrent ?? "";
  const e1PhaseIds = useMemo(() => (e1Gates?.phases ?? []).map((phase) => phase.p).filter(Boolean), [e1Gates]);
  const e1PhaseLabel = useCallback((phaseId: string) => {
  ...
  const skuPhaseIds = useMemo(() => {
    ...
    if (form.unlock && !ids.includes(form.unlock)) {
      ids.push(form.unlock);
    }
    return ids;
        ...
        <div className="col" style={{ gap: 12 }}>
          ...
          <SkuFieldGroup n="①" title="基本信息">
            ...
            <div className="grid g-2" style={{ gap: 12 }}>
              <label className="col" style={{ gap: 5 }}><span className="muted tiny">商品档位</span><select className="fld" value={form.tier} onChange={(e) => setForm({ ...form, tier: e.target.value })}>{SKU_TIER_OPTIONS.map((x) => <option key={x.value} value={x.value}>{x.label}</option>)}</select></label>
              <SkuFld label="营销角标" value={form.badge} onChange={(v) => setForm({ ...form, badge: v })} placeholder="热卖 / 新一代 / 趋势款" />
            </div>
            <SkuFld label="商品标语" value={form.tagline} onChange={(v) => setForm({ ...form, tagline: v })} placeholder="个人 AI 推理主机 · 全托管" />
            <div className="grid g-2" style={{ gap: 12 }}>
          ...
          <SkuFieldGroup n="②" title="硬件规格">
            ...
            <SkuFld label="数据中心" value={form.datacenter} onChange={(v) => setForm({ ...form, datacenter: v })} placeholder="新加坡机房" />
          </SkuFieldGroup>
          ...
          <SkuFieldGroup n="⑥" title="代际 & 生命周期">
            ...
            {form.tier !== "Share" && <>
              ...
              <SkuFld label="替代 SKU" value={form.supersededBy} onChange={(v) => setForm({ ...form, supersededBy: v })} placeholder="stellarbox-pro-v2(下一代内部标识)" hint="可空" />
            </>}
