      ...
      <div className="f4-grid">
        ...
        <section className="sect">
          <div className="sect-h">
            ...
            <div className="t"><div className="nm">区域大使确认</div><div className="s">V5+ · 4 类预算 · 待确认 7 单</div></div>
          ...
          <div className="sect-foot">
            <button className="primary amp" disabled={!!ambState} onClick={() => ctx.openActionConfirm({ name: "区域大使申请确认通过", amplify: true, op: "dispose", paramKey: "F.ambassador.q3-2025.status", fixedVal: "approved", status: "approved", detail: "批准大使申请 · 开通 4 类预算额度与权益 · 写 A2 审计 · 资金流出受 B1 约束。" })}>确认通过</button>
            <button className="danger" disabled={!!ambState} onClick={() => ctx.openActionConfirm({ name: "区域大使申请驳回", op: "dispose", paramKey: "F.ambassador.q3-2025.status", fixedVal: "rejected", status: "rejected", detail: "驳回大使申请 · 不开通预算 · 写 A2 审计。" })}>驳回</button>
          </div>
        </section>
      ...
      <section className="sect votes-card">
        ...
        <div className="vote-table">
          {V_VOTES.map((r) => {
            ...
            return (
              <div key={r.v} className="vote-cell" onClick={() => ctx.openActionConfirm({ name: `领导池 ${r.v} 票数权重调整`, amplify: true, op: "param", paramKey: `F.pool.votes.${r.v}`, edit: { kind: "number", current: eff, unit: "票" }, detail: `${r.v} 当前 ${eff} 票 · 改变领导池分配权重 · 调高放大头部虹吸 · 受 B1 覆盖率约束。` })}>
