...
function phaseName(code?: string) {
  return `${code ?? "P3"} ${RHYTHM_PHASE_NAME[code ?? "P3"] ?? ""}`.trim();
}
...
export default function H1Phase({ ctx }: { ctx: HCtx }) {
  ...
  const currentMonth = rhythm?.currentMonth ?? monthlyRows.find((row) => row.current)?.month ?? 1;
  const currentPhase = rhythm?.currentPhase ?? monthlyRows.find((row) => row.current)?.phase ?? "P3";
  const coverageRatio = text(model?.coverage?.coverageRatio, "-");
  const redlinePct = text(model?.coverage?.redlinePct, "-");

  const stats = useMemo(() => {
    const current = monthlyRows.find((row) => row.month === currentMonth);
    return {
    ...
    <>
      <div className="f-stats">
        ...
        <div className="f-stat cyan">
          ...
          <div className="sub">来源: nx_config_item growth.phase.month.*</div>
        </div>
            ...
            <table className="l-tbl" style={{ minWidth: 520 }}>
              ...
              <tbody>
                {model.attribution.map((row) => {
                  ...
                  return (
                    <tr key={phase} style={phase.includes(currentPhase) ? { background: "rgba(255,107,53,.08)" } : undefined}>
                      <td style={{ fontWeight: 600, color: "var(--ink)" }}>{phase}{phase.includes(currentPhase) ? " · 当前" : ""}</td>
