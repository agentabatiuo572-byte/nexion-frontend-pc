...
function statusMeta(value: unknown) {
  return TASK_STATUS[text(value, "active")] ?? TASK_STATUS.active;
}
    ...
    <>
              ...
              <table className="l-tbl" style={{ minWidth: 680 }}>
                ...
                <tbody>
                  {model.dayOneTasks.map((task, index) => {
                    ...
                    const active = text(task.status, "active") === "active";
                    return (
                      <tr key={id}>
      ...
      <section className="l-card">
        <div className="l-h">
          ...
          <div className="r">
            <span className={`bdg ${text(promoBanner.status, "active") === "active" ? "ok" : "dim"}`}>
              {text(promoBanner.status, "active") === "active" ? "上架中" : "已下架"}
            </span>
          </div>
        </div>
        <div className="l-b" style={{ paddingTop: 4 }}>
          ...
          <div className="p-row">
            ...
            <span className="v">{text(promoBanner.status, "active") === "active" ? "上架中" : "已下架"}</span>
            <button
              className="l-btn sm mc"
              onClick={() => openSimpleConfig("promoBanner.status", "转化卡上下架", text(promoBanner.status, "active") === "active" ? "paused" : "active", false)}
            >
              {text(promoBanner.status, "active") === "active" ? "下架" : "上架"}
            </button>
          </div>
          <div className="htint" style={{ marginTop: 10, fontSize: 12 }}>
