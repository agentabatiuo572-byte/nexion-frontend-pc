    ...
    <>
      <div className="f-stats">
        ...
        <StatCard
          ...
          sub={`短锁 ${toNumber(overview?.stats?.lockedShort)} · 长锁 ${toNumber(overview?.stats?.lockedLong)} · 来自 nx_user_security`}
        />
        {captchaOff ? (
      ...
      <div className="sec-grid">
        ...
        <section className="l-card">
          <div className="l-h"><span className="ttl">连错锁定</span><span className="sub">· 改动写 nx_config_item，解锁在 C5</span></div>
          <div className="l-b" style={{ paddingTop: 4 }}>
      ...
      <p className="f-foot">
        <b>全部在服务器执行</b>：验证码的有效期、试错次数、幂等去重都在服务端校验；锁定期间服务器拒绝该手机号的一切登录请求。数据源：{(overview?.sources ?? ["nx_config_item:auth.risk.*", "nx_user_security", "K1 facade boundary"]).join(" · ")}
      </p>
    </>
  );
}
