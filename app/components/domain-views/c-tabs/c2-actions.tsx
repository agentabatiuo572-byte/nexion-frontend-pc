...
function statusOf(account: User360Profile) {
  return text(account.status, "ACTIVE").toUpperCase();
}
...
export function C2Actions({ ctx }: { ctx: CCtx }) {
  ...
  const freeze = (account: User360Profile) => openActionConfirm({
    ...
    detail: "确认后写入后端账户状态,冻结动作同步吊销该用户全部活跃会话。刷新后以 nx_user / nx_user_session 查询结果为准。",
    amplifies: false,
    run: (reason) => {
      const id = accountId(account);
  ...
  const addList = (kind: "ALLOW" | "BLOCK") => openActionConfirm({
    ...
    detail: "输入用户编码或后端ID。确认后写入 nx_account_list,刷新后以服务端名单表为准。",
    amplifies: false,
    edit: { kind: "text", current: accounts[0]?.userNo ?? "U00000000", unit: "用户编码" },
    run: (reason, value) => {
      void perform(async () => {
  ...
  return (
    <>
      <div className="f-stats">
        <div className="f-stat danger"><div className="k">冻结中账户</div><div className="v">{loading ? "…" : frozenUsers}</div><div className="sub">来自 nx_user 状态统计</div></div>
        ...
        <div className="f-stat"><div className="k">信任名单</div><div className="v">{loading ? "…" : asNumber(overview?.trustListCount)}</div><div className="sub">nx_account_list · ALLOW</div></div>
        <div className="f-stat"><div className="k">禁入名单</div><div className="v">{loading ? "…" : asNumber(overview?.blockedListCount)}</div><div className="sub">nx_account_list · BLOCK</div></div>
      </div>
                ...
                {accountPager.pageRows.map((account) => {
                  ...
                  return (
                    <tr className={`click${locked ? " frozen-row" : ""}`} key={accountId(account)} onClick={() => setAcct(account)}>
                      ...
                      <td><span className={`bdg ${riskTone(account.riskScore)}`}>{text(account.riskScore, "0")}</span></td>
