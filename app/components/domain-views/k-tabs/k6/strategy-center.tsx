...
import { blankStrategy } from "@/lib/mock/admin/janus-c2/strategies";
import {
  ACTION_TYPE_LABEL,
...
import { ROLE_ORDER } from "@/lib/mock/admin/janus-c2/transitions";
              ...
              {canWrite && (
                pendingDel === s.strategyId ? (
                  <div className="k6-strat-actions">
                    ...
                    <button className="k6-pgbtn" style={{ color: "var(--danger)", borderColor: "var(--danger)" }} onClick={() => { del(s.strategyId, operator.id); setPendingDel(null); }}>确认删除</button>
                    <button className="k6-pgbtn" onClick={() => setPendingDel(null)}>取消</button>
                  </div>
                ) : (
                  <div className="k6-strat-actions">
                    <button className="k6-pgbtn" onClick={() => setEditing({ strategy: s, isNew: false })}>编辑</button>
                    <button className="k6-pgbtn" onClick={() => dup(s.strategyId, operator.id)}>复制</button>
                    ...
                    {s.status === "active" && <button className="k6-pgbtn" onClick={() => setStatus(s.strategyId, "paused", operator.id)}>暂停</button>}
                    {(s.status === "active" || s.status === "paused") && <button className="k6-pgbtn" onClick={() => setStatus(s.strategyId, "archived", operator.id)}>归档</button>}
