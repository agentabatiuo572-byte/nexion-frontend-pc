 ...
 * 多字段表单:模板 / 名称 / 说明 / 负责人 / 状态 / 优先级 / 命中动作 / 远程地址 / 适用范围 / 保护条件。
 * 🔴 状态可编辑(草稿/生效/暂停/归档);可枚举值(状态/动作/渠道)用 select/chip 不手输。
...
import { STRATEGY_TEMPLATES, strategyFromTemplate } from "@/lib/mock/admin/janus-c2/strategies";
import { dryRunStrategy, type DryRunResult } from "@/lib/mock/admin/janus-c2/evaluate";
import {
  ACTION_TYPE_LABEL,
  CHANNEL_LABEL,
...
import type { Strategy, StrategyActionType, StrategyStatus } from "@/lib/mock/admin/janus-c2/types";
...
const STATUS_OPTIONS: StrategyStatus[] = ["draft", "active", "paused", "archived"];
...
export function StrategyEditor({ initial, isNew, operatorId, onClose }: { initial: Strategy; isNew: boolean; operatorId: string; onClose: () => void }) {
  ...
  const onSave = () => { if (!valid) return; save(strategyForSubmit(), operatorId, isNew); onClose(); };
    ...
    <div className="k6-modal-overlay" onClick={onClose}>
      <div className="k6c2 k6-modal k6-editor" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label={isNew ? "新建策略" : "编辑策略"}>
        ...
        <div className="k6-modal-body">
          ...
          <div className="k6-form-row">
            ...
            <div className="k6-ovr-field" style={{ marginBottom: 0 }}>
              ...
              <select id="st-status" className="k6-field" value={s.status} onChange={(e) => patch({ status: e.target.value as StrategyStatus })}>
                {STATUS_OPTIONS.map((st) => <option key={st} value={st}>{STRATEGY_STATUS_LABEL[st]}</option>)}
              </select>
            </div>
            <div className="k6-ovr-field" style={{ marginBottom: 0 }}>
          ...
          <div className="k6-dryrun">
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <button className="k6-pgbtn" onClick={() => setDry(dryRunStrategy(strategyForSubmit(), devices, allStrategies))}>干跑预估</button>
              <span className="k6-hint" style={{ marginTop: 0 }}>对当前 {devices.length} 台设备模拟评估规则树,不产生真实下发{dry ? ` · 命中率 ${dry.hitRate}% · 冲突 ${dry.conflicts} 台${dry.other > 0 ? ` · 其他动作 ${dry.other} 台` : ""}` : ""}。</span>
            </div>
            {dry && (
        ...
        <div className="k6-ovr-foot">
          ...
          <button className="k6-ovr-confirm" disabled={!valid} onClick={onSave}>{isNew ? "创建策略" : "保存修改"}</button>
        </div>
      </div>
    </div>
  );
