...
export function VersionPanel({ strategy: s, operatorId, canRollback, onClose }: { strategy: Strategy; operatorId: string; canRollback: boolean; onClose: () => void }) {
  ...
  const confirmRollback = async () => {
    if (!pending || !reasonValid || rollingBack) return;
    setRollingBack(true);
    setRollbackError("");
    try {
  ...
  return (
    <div className="k6-modal-overlay" onClick={() => { if (!rollingBack) onClose(); }}>
        ...
        {pending ? (
          <>
            <div className="k6-modal-body">
              ...
              {rollbackError && <div className="k6-hint" style={{ color: "var(--danger)", marginTop: 8 }}>回滚失败：{rollbackError}</div>}
            </div>
            <div className="k6-ovr-foot">
              <button className="k6-ovr-cancel" onClick={() => setPending(null)}>返回</button>
              <button className="k6-ovr-confirm" disabled={!reasonValid || rollingBack} onClick={() => void confirmRollback()}>{rollingBack ? "回滚中…" : "确认回滚"}</button>
            </div>
          </>
        ) : (
          <>
