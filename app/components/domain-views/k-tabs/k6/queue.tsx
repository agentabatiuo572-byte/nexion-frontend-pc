...
import { NOW_MS, timeAgo } from "@/lib/mock/admin/janus-c2/scoring";
import { STATUS_LABEL, STATUS_SOURCE_LABEL, STATUS_TONE, SUGGESTED_ACTION, channelLabel } from "@/lib/mock/admin/janus-c2/labels";
import type { Device, DeviceStatus } from "@/lib/mock/admin/janus-c2/types";
...
export function K6Queue() {
  ...
  const channels = useMemo(() => [...new Set(JANUS_DEVICES.map((d) => d.channel).filter(Boolean))] as string[], []);
  const strategies = useMemo(() => [...new Set(JANUS_DEVICES.map((d) => d.hitStrategy).filter(Boolean))] as string[], []);
  ...
  const filtered = useMemo(() => {
    ...
    const rows = devices.filter((d) => {
      ...
      if (time === "today" && d.lastSeenAt < NOW_MS - (NOW_MS % DAY_MS)) return false;
      if (time === "24h" && NOW_MS - d.lastSeenAt > DAY_MS) return false;
      if (time === "7d" && NOW_MS - d.lastSeenAt > 7 * DAY_MS) return false;
      if (op !== "all") {
            ...
            <tbody>
              {pageRows.length === 0 ? (
                ...
                <tr key={d.sid} className={`click${selected === d.sid ? " sel" : ""}`} onClick={() => setSelected(d.sid)}>
                  ...
                  <td><span className={`k6-bdg ${STATUS_TONE[d.status]}`}>{STATUS_LABEL[d.status]}</span></td>
