...
export const K2_VIEWS: Record<K2View, { label: string; sub: string; head: string[]; note: string; rows: K2Row[] }> = {
  ...
  tradein: {
    ...
    note: "「产出比」= 该设备累计产出 ÷ 实付价,决定置换抵扣落在哪一档(E3 配置,这里只读)。单笔置换无套利面(抵扣仅结算抵减、不入余额、仅限升级更高价,平台每笔净收新款);本视图盯的是高频循环 + 礼金/返佣叠加的闭环信号,标记观察为主。",
    rows: [
...
// PRD K3③ 四维规则卡(条件 / 命中动作 / 调参 key);key 落 K.rule.<ruleKey>(沿用既有真写键,结构化表单提交后存整条规则文案)。
...
export const K5_TICKETS: K5Ticket[] = [
  {
    ...
    info: [["触发原因", "单笔提现 $8,200 ≥ $1,000"], ["提现单", "WD-90412 · 复审未决(D2 待人工 · 禁放)"], ["实名材料", "服务商档案 #SB-44102 · 2026-01 提交"], ["风险评分(K4)", `${r31E8} · ${r31E8 >= 70 ? "高" : r31E8 >= 40 ? "中" : "低"}`], ["账户年龄", "181 天"], ["历史提现", "41 笔 · 全部正常"]],
    hist: [["昨天 14:22", "触发 · 单笔大额(K3 命中同刻)", ""], ["昨天 14:22", "提现单 WD-90412 进入复审未决(复审未过维持待确认 · 禁放)", "warn"], ["今天 10:05", "材料初核完成 · 等待裁决", ""]],
  },
  {
