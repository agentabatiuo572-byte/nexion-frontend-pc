...
export function K3Rules({ ctx }: { ctx: KCtx }) {
  ...
  return (
    <div>
      ...
      <p className="f-foot">
        <b>所有评估都在服务器</b>:提现请求到达时服务端逐条过规则,客户端没有任何办法跳过;KYC 地址匹配、NEX 闸这些既有门槛照常二次校验,这里的路由结论叠加在它们之上。命中冻结的提现单进「冻结」状态(客户端只能看),延迟的延长审核停留,转人工的进 D2 队列由人分诊 —— D2 同时展示风险评分(K4)+ 命中规则(本页),两套信号配合分诊。<b>提现冷却天数、NEX 闸不在这里设</b> —— 那是运营节奏参数,归 H1 派发、在提现配置(D5)生效。命中事件同时喂风险雷达(B5)。
      </p>
