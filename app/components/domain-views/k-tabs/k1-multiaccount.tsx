          ...
          {overview && overview.releaseParams.length > 0 ? (
            <>
              ...
              <div className="ktint warn" style={{ marginTop: 12 }}>
                <b>落地规则</b> · 正常槽位内收益可提;超过待审起点进入审核中;达到重复账号冻结线(第 N 个账号)或人工冻结后进入锁定奖励。审核中的收益<b>不随时间自动放行</b>:释放只认 App 在线证明达标或人工放行两个来源;观察窗口内同簇批量释放有熔断——超出正常槽位数的待审收益自动升为锁定奖励。放宽任一参数 = 放大资金流出方向,请在操作理由写明依据。
              </div>
            </>
          ) : (
