...
export function A4Events({ ctx }: { ctx: ACtx }) {
  ...
  const paramValue = (key: string, fallback: string) => KPI_DIMENSION_PARAMS.find((param) => param.key === key)?.value ?? fallback;
  const liveSchemaVer = A4_STATS.schemaVersion;
  const liveDay0 = paramValue("day0", "90 秒");
  const liveEventRetention = paramValue("event_retention", "13 个月");
  const liveSampling = paramValue("sampling", "浏览 10% · 资金 100%");
