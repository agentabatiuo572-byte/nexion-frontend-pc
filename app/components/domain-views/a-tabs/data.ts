...
export const FEATURE_FLAGS = [
  { key: "ab.newWithdrawFlow", name: "新版提现流程实验", desc: "新提现页与旧版 A/B,对比转化与流失", st: "灰度 20%", scope: "全量随机", lastChange: "06-09 · 高翔/陈锐", resourceOwner: "增长可发起", growthClass: true },
  { key: "ab.homeBannerExp", name: "首页 Banner 实验", desc: "首页活动横幅文案/版式 A/B", st: "on", scope: "注册满 20 周", lastChange: "06-05 · 高翔/陈锐", resourceOwner: "增长可发起", growthClass: true },
  { key: "exp.questBoostAB", name: "任务奖励加成实验", desc: "P3 阶段任务奖励加成 A/B", st: "on", scope: "P3 阶段用户", lastChange: "06-02 · 高翔/陈锐", resourceOwner: "增长可发起", growthClass: true },
  { key: "core.sse_v2", name: "实时推送通道 v2", desc: "配置失效广播新链路(SSE v2)灰度", st: "灰度 50%", scope: "全量随机", lastChange: "05-28 · 陈锐/赵敏", resourceOwner: "超管(平台能力)", growthClass: false },
...
export const killSwitchReadonly = (): { key: string; name: string; st: string; lastChange: string; chain: string }[] => {
  const fns = KILLSWITCH.map((k) => ({
    key: k.key,
    name: k.name, // 运营可读中文名(KILLSWITCH 单源:提现 / Genesis 经济 / NEX 兑换 …);key 作技术副标
  ...
  return [
    ...fns,
    { key: "geo-block", name: "地区屏蔽", st: "空列表 · 无封锁", lastChange: "—", chain: "— / —" },
  ];
};
