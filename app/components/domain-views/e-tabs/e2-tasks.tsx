...
/** Onboarding 比较行的服务器权威来源；商品收益在 E1，手机收益在本页 T3 调整。 */
...
export function E2Tasks({ ctx }: { ctx: EViewCtx }) {
  ...
  const canMutateTasks = ctx.canWriteE2 && !ctx.e2Loading && ctx.e2TaskCatalogReady;
  ...
  return (
    <>
      ...
      <section className="pane" data-testid="e2-onboarding-yield-comparison">
        <div className="pane-h">
          <span className="ttl">Onboarding 收益对比配置</span>
          <span className="sub">App 校准结果同源</span>
        </div>
        <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
          {!ctx.e2Loading && ctx.yieldComparisons.length !== 4 && <div className="tint danger tiny">来源配置不完整，请检查 E1 在售商品与手机 T3 档位。</div>}
          {ctx.yieldComparisons.map((row) => {
            return (
            <div key={row.configKey} style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 14, alignItems: "center", paddingTop: row.sortOrder === 1 ? 0 : 10, borderTop: row.sortOrder === 1 ? "none" : "1px solid var(--border)" }}>
              <div className="col" style={{ gap: 2 }}>
                ...
                <span className="tint tiny">{row.configKey === "phone" ? "来源：手机 T3 档位" : `来源：E1 商品 ${E2_COMPARISON_SKU_ID[row.configKey]}`}</span>
              </div>
              <div className="tnum" style={{ textAlign: "right", fontSize: 12.5 }}>${amount(row.dailyUsdt)} · {amount(row.dailyNex)} NEX/天</div>
              <span className="tint tiny">请在来源配置调整</span>
            </div>
            );
          })}
        </div>
            ...
            {pageRows.length === 0 ? (
              ...
              return (
                <div className="task" key={t.id}>
                  ...
                  {canMutateTasks && <div className="acts">
