...
function kpiTargetLabel(kpi: Record<string, unknown>) {
  ...
  if (rawText(kpi.dir) === "band") {
    const [low = 0, high = 0] = rawNumRows(kpi.band);
    return `${low}-${high}${unit}`;
  }
  const sign = rawText(kpi.dir) === "lte" ? "<=" : ">=";
  return `${sign}${rawNum(kpi.target)}${unit}`;
}

function kpiPass(kpi: Record<string, unknown>) {
  const value = rawNum(kpi.value);
  const target = rawNum(kpi.target);
  const dir = rawText(kpi.dir, "gte");
  if (dir === "band") {
    const [low = 0, high = 0] = rawNumRows(kpi.band);
...
function normalizeDashboardKpis(data: LBiData | null): DashboardKpi[] {
  ...
  return rawRows(l1.kpis).map((row, index) => {
    const n = rawNum(row.n, index + 1);
    const ext = rawRec(kpiExt[String(n)]);
    const spark = rawNumRows(row.spark);
    const unit = rawText(row.unit);
    const label = rawText(row.name, `KPI #${n}`);
    return {
      key: `l1-${n}`,
      label,
      value: `${rawNum(row.value)}${unit}`,
      target: kpiTargetLabel(row),
      pass: kpiPass(row),
      series: spark.length ? spark : [rawNum(row.value)],
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 text-[12.5px]" style={{ color: "var(--v5-ink-3)" }}>
            {isLoading ? "正在读取服务端接口。" : error || "接口返回为空,未使用静态 mock 兜底。"}
          </div>
        </div>
      </div>
    </section>
...
export default function CommandCenter() {
      ...
      <header className="mb-5 flex flex-wrap items-end gap-4">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1.5 text-[13.5px]" style={{ color: "var(--v5-ink-2)" }}>
            欢迎,{operator}<AutoGloss> · 正在读取服务端 B 域资金、漏斗、节奏和风险聚合数据。</AutoGloss>
          </p>
        </div>
      </header>
  ...
  const sensitiveOperationItems: SensitiveOperationItem[] = [
    ...
    ...pendingA2Operations.map((row) => ({
      id: `a2-${row.id}`,
      label: row.action,
      detail: operationDetail(row),
      href: "/platform/audit",
      role: roleForA2Operation(row),
    })),
    ...
    <div className="w-full">
      ...
      <header className="mb-5 flex flex-wrap items-end gap-4">
        <div className="min-w-0 flex-1">
          ...
          <h1 className="font-display mt-1.5 text-[26px]" style={{ color: "var(--v5-ink)" }}>运营总览</h1>
          <p className="mt-1.5 text-[13.5px]" style={{ color: "var(--v5-ink-2)" }}>
            欢迎,{operator}<AutoGloss> · 资金兑付安全和转化健康一屏看全 · 以服务端数据为准。</AutoGloss>
          </p>
        </div>
        <div className="flex items-center gap-2.5">
          <button
