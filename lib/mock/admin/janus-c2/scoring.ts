 ...
 * 时间基准用固定快照时钟 NOW_MS(非 Date.now())→ SSR/CSR 渲染一致,无水合错位。
 */
import type { C2Summary, Device, RecommendationLevel } from "./types";

/** 固定快照时钟:略晚于数据集中最新 lastSeenAt(2026-06-26)。 */
export const NOW_MS = Date.parse("2026-06-26T18:40:00.000Z");
const FRESH_WINDOW_MS = 5 * 60 * 1000;

export function isFresh(lastSeenAt: number): boolean {
  return NOW_MS - lastSeenAt <= FRESH_WINDOW_MS;
}
...
export function installDaysOf(installAt: number): number {
  return Math.max(0, Math.floor((NOW_MS - installAt) / 86_400_000));
}
...
export function computeRecommendationScore(d: Device): number {
  ...
  // 关键动作:optimize 5(PRD §10.2 optimize/claim/detail 各 5;mock 仅建模 optimize 一项)
