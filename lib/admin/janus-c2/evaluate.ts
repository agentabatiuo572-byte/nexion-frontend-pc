...
function strategyApplicability(strategy: Strategy, d: Device): { passed: boolean; leaves: DecisionRuleResult[] } {
  ...
  if (rollout) {
    ...
    if (rollout.startedAt && rollout.startedAt > now) leaves.push(blocked("灰度开始时间", "尚未开始"));
    if (rollout.endsAt && rollout.endsAt < now) leaves.push(blocked("灰度结束时间", "已结束"));
  ...
  if (freshMinutes != null) {
    const elapsed = Math.max(0, Math.round((now - d.lastSeenAt) / 60_000));
  ...
  if (minHours != null) {
    ...
    const elapsedHours = lastAt ? (now - lastAt) / 3_600_000 : undefined;
