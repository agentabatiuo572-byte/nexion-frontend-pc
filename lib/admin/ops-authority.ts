 ...
 * 不变量:超管(super)恒可执行;其余角色须命中 gate.roles(+ requireLead 时须 lead 层级)。
...
export interface ExecGate {
  ...
  requireLead?: boolean; // true = 该角色须 lead 层级才能执行
}

export function canExecute(op: ActingOperator, gate: ExecGate): boolean {
  if (op.role === "super") return true; // 超管恒可
  if (!gate.roles.includes(op.role)) return false;
  if (gate.requireLead && op.tier !== "lead") return false;
  return true;
}

/** 门槛展示文案,如「财务 lead / 超管」。超管总在末尾(恒可执行)。 */
export function gateLabel(gate: ExecGate): string {
  const parts = gate.roles.map((r) => ROLE_LABEL[r] + (gate.requireLead ? " lead" : ""));
