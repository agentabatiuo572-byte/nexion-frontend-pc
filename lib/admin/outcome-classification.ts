/**
 * 高敏写操作失败的**统一归类口径**(判据单源)。
 *
 * 🔴 「统一」指的是**判据只此一处**,不等于「每个域都接上了」(2026-08-06 收尾审计纠正)。
 *   接线现状以 `tests/outcome-classification-contract.test.mjs` 的覆盖面台账为准:
 *   没接的域要么走通用执行器(由 stable-mutation 统一归类),要么根本没有可复用的命令号
 *   ——后者归类对了也没号可保,属交接文档「任务 A」的迁移范围。
 *
