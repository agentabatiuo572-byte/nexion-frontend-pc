...
export const DOMAIN_K: ModuleEntry[] = [
  ...
  },
  {
    ...
    summary: "提现风控规则引擎(K3)。声明从金额、速度、新账户、收款地址信誉四个维度判定,给出放行 / 延迟 / 冻结 / 转人工四种结果;D2 照这个结果执行,而且优先级高于小额快速通道。服务端评估器支持全部四个维度,但只有已配置生效规则的维度才真正参与判定;缺维度时页面显式降级告警,不宣称四维规则引擎完整生效。增删改规则、启停规则都要确认;规则一旦归档就是终态(不能再激活),「放行」结果不另产事件。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
    path: "/risk/scoring",
    summary: "风险评分模型(K4)。五个维度按权重(加起来等于 1、两端都校验)合成 0–100 分:低危 <40、中危 40–69、高危 ≥70,≥85 自动建议转人工。这是全平台唯一的评分来源(D2/C1/B5 只引用、不各自重算),每个分都能解释来由;改权重或分档要超级管理员才能执行。",
    content: PORTED_EMPTY_CONTENT,
  },
];
