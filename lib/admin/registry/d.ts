...
export const DOMAIN_D: ModuleEntry[] = [
  ...
  {
    path: "/finance/params",
    summary: "提现参数配置(D5)。每日次数、余额上限、三网络固定确认费、NEX 抵扣率、小额免审和到账时效都由真实提现状态机消费；变更基于版本号原子提交并留审计，放大资金流出方向还要先过 B1 覆盖率红线。真实 BSC 终局证明未配置时资金保持在途，不会提前结算。到账审查窗口与加强合规审查由 H1 Phase 单一派发，本页只读展示并提供 H1 跳转。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
  ...
  },
  {
    ...
    summary: "法币提现参数(D7)。八项运营参数采用服务端权威整组配置，保存携带版本、幂等键与操作理由并写入 A2 审计；D6 基准价和买入点差只读复用。HDPay 银行提现已接入报价、审核、订单、账本与回调处理；真实出款须完成商户权限、回调配置、供应商语义确认及授权验收。供应商未就绪或状态不可用时拒绝开启，但存量通道的关闭止损入口保持可用。",
    content: PORTED_EMPTY_CONTENT,
  },
];
