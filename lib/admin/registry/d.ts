...
export const DOMAIN_D: ModuleEntry[] = [
  ...
  },
  {
    ...
    summary: "提现审核队列(D2)。两路信号只用、不重算:风险分(来自 K4 同一个分)、命中的规则(来自 K3)。提现状态由服务器统一推进——正常 5 种状态 + 异常 6 种状态。大额要走确认 + 先过 B1 备付金覆盖率预检；批量里夹着大额会自动拆成单笔逐一审。放行会实时扣减 D3 储备,并同步给 B1/B5。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
    path: "/finance/pool",
  ...
  {
    path: "/finance/params",
    summary: "提现参数配置(D5)。D5 统一管理六组非 Phase 参数:每日提现次数、小额免审线、到账时效、余额可提上限、网络费率+min/max、NEX 抵扣率；变更基于版本号原子提交并留审计，放大资金流出方向还要先过 B1 覆盖率红线。小额免审线决定多大金额能同时免掉「首次提现人工初审」和「新绑地址 24 小时延迟」两道闸(其余风控闸照常裁决)。到账审查窗口、提现惩罚费率、加强合规审查由 H1 Phase 单一派发，本页只读展示并提供 H1 跳转。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
  ...
  {
    path: "/finance/payout-vnd",
    summary: "法币提现参数(D7)。银行卡(越南盾)提现通道的配置面:卖出点差与双向牌价预览(基准价单源在 D6,本页只读)、出金报价有效期与重报价偏差阈值、提现费率(百分比+最低+封顶)与单笔上下限、通道总开关(首发默认关闭)。价差倒挂默认拒绝、仅超级管理员可强制;每次调整必须确认+理由并落变更历史;调整只影响新提现单,与 USDT 侧网络确认费(D5)两套参数各自独立。",
    content: PORTED_EMPTY_CONTENT,
  },
];
