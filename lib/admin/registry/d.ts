...
export const DOMAIN_D: ModuleEntry[] = [
  ...
  },
  {
    ...
    summary: "提现审核队列(D2)。两路信号只用、不重算:风险分(来自 K4 同一个分)、命中的规则(来自 K3)。提现状态由服务器统一推进——正常 5 种状态 + 异常 6 种状态。大额要走确认 + 先过 B1 备付金覆盖率预检；批量里夹着大额会自动拆成单笔逐一审。银行卡提现放行时保留提现资金占用，核实 HDPay 付款成功后才扣减 D3 储备，并同步给 B1/B5。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
    path: "/finance/pool",
  ...
  },
  {
    ...
    summary: "法币提现参数(D7)。八项运营参数采用服务端权威整组配置，保存携带版本、幂等键与操作理由并写入 A2 审计；D6 基准价和买入点差只读复用。HDPay 银行提现已接入报价、审核、订单、账本与回调处理；代收与代付共用 HDPay 商户配置，不再维护独立通道开关。",
    content: PORTED_EMPTY_CONTENT,
  },
];
