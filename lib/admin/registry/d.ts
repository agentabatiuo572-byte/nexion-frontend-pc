...
export const DOMAIN_D: ModuleEntry[] = [
  ...
  },
  {
    ...
    summary:
      "提现审核队列(D2)。三路信号只用、不重算:风险分(来自 K4 同一个分)、命中的规则(来自 K3)、实名状态(来自 C4)。提现状态由服务器统一推进——正常 5 种状态 + 异常 6 种状态。低于大额线且低风险的,普通确认就能快速放行,守住 48 小时到账承诺;达到大额线要走确认 + 先过 B1 备付金覆盖率预检。被 K5 拉去复审、还没过的单子禁止放行;批量里夹着大额会自动拆成单笔逐一审。放行会实时扣减 D3 储备,并同步给 B1/B5。",
