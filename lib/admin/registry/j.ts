...
export const DOMAIN_J: ModuleEntry[] = [
  {
    ...
    summary:
      "5 大业务的紧急熔断开关矩阵(前端 4 个 + 后台应急加的提现闸;Premium/NEX v2 已下线)。一拉闸全站立即生效、客户端绕不过;拉闸和恢复都要确认并记入 A2,恢复那些「会往外付钱」的闸之前要先过 B1 备付金核验。",
    content: {
      kind: "config",
      metrics: [
  ...
  },
  {
    ...
    summary:
      "按国家 / 地区的准入控制:黑名单(全功能封禁)和受限名单(只读)两档;封锁按访问 IP 在服务器入口判定(和「风险提示书」用的 IP+国籍判定是两套独立机制)。改名单要风控主管或超级管理员确认执行,财务不参与。",
    content: {
      kind: "config",
      metrics: [
  ...
  },
  {
    ...
    summary:
      "篡改防御监控(纯查看页,不动手)。余额、产出、节点归属、价格等关键值都以服务器为唯一权威,客户端想改会被服务器当场驳回并计数;频繁篡改的账户会把信号喂给 K4 风险评分,真正的处置去 C2 / K1 完成。",
    content: {
      kind: "list",
      metrics: [
  ...
  },
  {
    path: "/emergency/sop",
    summary:
      "监管点名 / 突发事件的应急预案库。每个预案由一连串跨域动作编排而成(熔断、封锁、披露、冻结、限流、通知),每一步都要过对应域的确认;支持沙箱演练和实战执行,全程留痕、可审计可回溯。",
    content: {
      kind: "list",
      metrics: [
