...
export const DOMAIN_I: ModuleEntry[] = [
  ...
  {
    path: "/content/nova",
    summary: "Nova 站内 AI 助手的推送运营页(I2):可调通道节奏表(逐条由服务端 nx_nova_channel 目录派生,含 v3 业务频道 team_event / staking_event / market_event 的开关与服务器 cadence)+ 单个通道的开关 + 模板池 + 真实事件池(5 类按概率派发、合计 100%)。关单个通道不算应急熔断(不占 J1/J2 闸位)、也不是地区屏蔽——停某个频道的推送在本页确认即可;只有「要把整个 Nova 能力停掉」才轮到 J 域出手。两个随阶段变的通道(以旧换新提醒、月度任务锁提醒)由 H1 节奏说了算,本页只读跟随。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
  ...
  {
    path: "/content/trust",
    summary: "信任中心内容管理(I4):用户端 /trust 页的 6 个版块(财务数字 / 团队 / NEX 故事 / 徽章 / 审计 / 外链)。草稿由内容角色直接保存；发布、回滚和下架按普通版块与敏感版块分权，财务数字 / NEX 叙事发布时必须填写数据来源并完成中越英三语核对。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
    path: "/content/disclosures",
    summary: "风险披露版本管理(I5):按法域 × 7 章节管理披露矩阵、版本快照、用户重新确认覆盖监控和受限动作范围；法域数量以服务器配置目录为准。披露由风控起草，风控主管或超级管理员发布；重新确认不是熔断开关，不占 J1/J2 闸位。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
    path: "/content/i18n",
