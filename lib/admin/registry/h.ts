...
export const DOMAIN_H: ModuleEntry[] = [
  ...
  },
  {
    ...
    summary: "对外公布数据(H9)—— 管理首页公布的平台规模，包括设备总数、在线占比与展示浮动、注册用户基数与月增长率。虚拟人口和算力分位表保留用于历史本地估算，不改变正式 App 当前服务端名次。设备总数同时供介绍页、信任页与分享海报使用，并影响对外公布的日支付额；全球网络读取独立区域投影。页面在保存前给出影响预览。7 个参数与分位表整组原子保存：确认弹窗给前后值对照、理由必填、服务端落审计并带幂等键，任一项不合法整组不落库。",
    content: PORTED_EMPTY_CONTENT,
  },
];
