...
export const DOMAIN_H: ModuleEntry[] = [
  ...
  {
    // H9 历史参数仍由后端 /api/admin/growth/public-stats 保存并审计。
    path: "/growth/public-stats",
    summary: "历史估算配置(H9)—— 保留设备、注册基数、展示浮动、月增长率、虚拟人口与算力分位表等历史参数供内部估算，不参与正式 App 的在线设备、注册账户、已付金额或服务端名次。正式对外事实读取服务端业务聚合并附统计时刻。页面在保存前给出估算影响预览；参数与分位表整组原子保存，确认弹窗显示前后值，理由必填，服务端落审计并带幂等键。",
    content: PORTED_EMPTY_CONTENT,
  },
];
