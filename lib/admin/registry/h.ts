/**
 * 域 H 增长与运营节奏 — 注册表(H1 Phase / H2 试用 / H3 Quest / H4 活动 / H5 签到&里程碑)。accent=--admin-domain-h。
 * ⚠️ H ∈ PORTED_DOMAINS:本文件 content 为死代码(真渲染面 = h-view.tsx + h-tabs/),仅 summary 经 DomainHeader 渲染。
 * 改 H 域数据/动作请改后端 growth 接口与 h-tabs 页面组件,勿在此处改 content。
 * H3/H4 来自高保真同页顶部分段,菜单层已拆成两个独立入口;PC 端只保留 H5 承载签到与里程碑。
 */
...
export const DOMAIN_H: ModuleEntry[] = [
  {
    ...
    content: placeholder("死代码:H1 真渲染面在 h-tabs/h1-phase.tsx,数据来自 /api/admin/growth/phases。"),
  },
  {
    path: "/growth/trial",
    summary:
    ...
    content: placeholder("死代码:H2 真渲染面在 h-tabs/h2-trial.tsx,数据来自 /api/admin/growth/trials。"),
  },
  {
    path: "/growth/quest",
    summary:
    ...
    content: placeholder("死代码:H3 真渲染面在 h-tabs/h3-quest-events.tsx 的任务引擎 section,数据来自 /api/admin/growth/quest-events。"),
  },
  {
    path: "/growth/events",
    summary:
    ...
    content: placeholder("死代码:H4 真渲染面在 h-tabs/h3-quest-events.tsx 的活动中心 section,数据来自 /api/admin/growth/quest-events。"),
  },
  {
    path: "/growth/daily",
    summary:
      "每日签到引擎 · 6 条规则 + 幸运中奖概率倍率(随机抽奖在服务器跑)+ 30 天连续签到的 7 级里程碑 + 收益累计里程碑(5 档阈值和对应 NEX 奖励)+ 断签复活卡 + 4 档连签增益(联动 F2/G1/G4)。幸运两档的中奖概率之和不能超过 100% + 调高概率 / 奖励 / 增益阈值 / 调低收益门槛这类放大流出的改动要过 B1;实际中奖率和配置值相差超过 ±1 个百分点就报警;签到满 30 天发的转盘券归 H4 管。",
    content: placeholder("死代码:H5 真渲染面在 h-tabs/h5-daily-milestones.tsx,数据来自 /api/admin/growth/check-in。"),
  },
  {
