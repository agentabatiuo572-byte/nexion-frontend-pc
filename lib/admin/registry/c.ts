...
export const DOMAIN_C: ModuleEntry[] = [
  {
    path: "/users/search",
    summary: "C 域入口和用户查找页(C1):用多个条件找到人,一屏看全用户的分层、设备、风险分、余额、状态。这里只能查、不能改——数字都是从各域取来的真实值、不在本页重算;要处置去对应页面(冻结去 C2、资产去 C3、安全去 C5)。用户分层 L0–L5 和会员等级 V0–V12 只有运营看得到,用户端永远看不到。",
  ...
  {
    path: "/users/security",
    summary: "单个用户的账户安全处置页(C5):两步验证、登录会话、重置密码、解除锁定。核心是防止骗子套话夺号:关掉两步验证、重置密码都要经过高风险操作确认;密码只存加密后的、谁都看不到明文。锁定分两档解锁:短锁走安全校验,长锁要走确认;两档的触发次数与锁定时长都以 C6 当前服务端配置为准,本页只读展示锁定记录里的实时到期时间。解锁的处置权都在本页,而「什么情况下锁」的阈值在 C6 配。",
    content: PORTED_EMPTY_CONTENT,
  },
  {
    path: "/users/reg-risk",
