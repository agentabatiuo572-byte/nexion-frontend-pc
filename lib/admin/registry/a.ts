...
export const DOMAIN_A: ModuleEntry[] = [
  {
    path: "/platform/rbac",
    summary: "管后台账号、以及每个角色能做哪些操作(谁做过什么记在 A2)。本页核心是全域权限矩阵、登录与安全基线和默认拒绝原则。三条不能改的硬规矩:① 所有人登录都要两步验证,关不掉;② 新开的账号默认没有任何权限,要手动分配;③ 平台随时至少保留 2 个超级管理员,免得没人能管。开户、停用、启用、改角色、重置两步验证——只有超级管理员能做,且必须写明原因;强制把账号踢下线也只有超级管理员能做,点完立即生效、要写原因。改权限表要确认后才发布,系统会自动拦下越权的搭配。",
    content: placeholder,
  },
  {
    path: "/platform/audit",
