...
export const DOMAIN_A: ModuleEntry[] = [
  {
    path: "/platform/rbac",
    summary: "管后台账号、以及每个角色能做哪些操作(谁做过什么记在 A2)。三条不能改的硬规矩:① 所有人登录都要两步验证,关不掉;② 新开的账号默认没有任何权限,要手动分配;③ 平台随时至少保留 2 个超级管理员,免得没人能管。开户、停用、启用、改角色、重置两步验证——只有超级管理员能做,且必须写明原因;强制把账号踢下线也只有超级管理员能做,点完立即生效、要写原因。改权限表要确认后才发布,系统会自动拦下越权的搭配。",
    content: placeholder,
  },
  {
    path: "/platform/audit",
  ...
  },
  {
    ...
    summary: "系统配置(A3):管整个平台通用的底层开关。① 统一时间源——试用倒计时、阶段月龄、提现冷却、锁仓到期等一切时间判定都只认服务器,用户改自己手机/电脑的时钟也占不到便宜;② 防重复策略——资金和资产写入有 24 小时去重窗口,网络重试不会重复扣款;③ 灰度发布台——新功能先小范围放给一部分人试,放给谁由服务器决定、前端只能拿到结果;④ 熔断开关的状态存在这里(开关本身的操作已搬到应急域 J1/J2,本页只能看);⑤ 系统健康面板。",
    content: placeholder,
  },
  {
