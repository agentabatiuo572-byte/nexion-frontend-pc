...
const G2_CAP_KEYS = new Set(["userDailyCap", "platformDailyCap", "fee", "feeMin", "queueMode"]);
const G2_GATE_KEYS = ["user", "platform", "geo"] as const;
const G4_PARAM_KEYS = new Set(["supply", "price", "dividend", "royalty", "divBase", "airdropPct", "emissionCurve", "airdropLockDays", "showcaseEnabled"]);
...
export function assertG2OverviewContract<T>(value: T): T {
  ...
  const stats = record(module, root.stats, "data.stats");
  const statKeys = ["todayUsd", "poolPct", "queueDepth", "gateUser", "gatePlatform", "gateGeo"] as const;
