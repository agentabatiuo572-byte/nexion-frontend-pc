...
/**
 * 各域高敏动作接入 A2 实时 pending 的便捷 hook —— 一次绑定平台 store + pending store + 当前身份,
 * 域里只需 `const propose = usePropose();` 然后在 run 里 `propose(toast, spec)`:
 *   够执行门槛 → 直接执行(回放 spec.mutations + 审计);不够 → 入 pending 提案等有权者执行。
...
import { usePendingOps } from "@/lib/store/admin/pending-ops-store";
...
export function usePropose() {
  ...
  const addProposal = usePendingOps((s) => s.addProposal);
  return (toast: (s: string) => void, spec: ProposeSpec): "executed" | "proposed" =>
    proposeOrExecute({ acting, setParam, logAudit, addProposal, toast }, spec);
}
