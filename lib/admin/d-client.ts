...
export interface D2BankPayout {
  ...
  beneficiaryEligibility: BankBeneficiaryEligibility | null;
...
export async function fetchD2BankPayout(withdrawalNo: string): Promise<D2BankPayout> {
  ...
  return { withdrawalNo, state, version,
    ...
    beneficiaryEligibility: readBankEvidence(parseBankEligibility, row.beneficiaryEligibility), settlementEvidence: readBankEvidence(parseBankSettlement, row.settlementEvidence) };
}
