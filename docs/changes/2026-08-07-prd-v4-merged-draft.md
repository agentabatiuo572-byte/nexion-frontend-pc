# NexGrid · 运营控制后台 PRD(Ops Console PRD)— V4 卷:内容合规 CMS(I)+ 紧急合规(J)+ 数据 BI(L)+ 全局总收口

<!-- ===================================================================
  合并稿 · 2026-08-07 · 未签字草稿,勿直接替换任一原件。
  来源 A(权威副本): PLAN/PRD/NexGrid_运营控制后台PRD_v4.md
  来源 B(仓内副本): PLAN/admin-ops/docs/PRD/Nexion_运营控制后台PRD_v4.md
  合并规则:只在一边出现的内容全部保留;事实/口径分叉不自行拍板,就地
  保留推荐版并用「冲突 #N 待裁决」HTML 注释就近标注,全文共 17 处冲突。
  品牌统一 NexGrid;Nexion-janus / Nexion-Janus_*.md / nexion-admin-prd
  等标识符与真实文件路径按接口契约原样保留,未改名。
  冲突逐条对照与决策依据见 2026-08-07-prd-v4-merge-report.md
=================================================================== -->
...
**① 目的 & 对齐**
配置平台转化文案池(MissedIncome / ConversionBanner 等)的版本管理、A/B 实验框架与上下架,作为「转化优先」(**§1.4 运营优先级**:后台第一优先级是支撑用户向购买 NexGridBox 的转化与平台资金兑付安全;商业模式四支柱收入见 §1.5)的文案运营面。对齐前端 **§9.11c.2**(`GET /admin/home/conversion-banner.copy`「主 CTA 跨平台,A/B 优先」+ Home ConversionBanner「this week's bonus + payback 天数 + ROI anchor」)+ **§6.6**(MissedIncomeBanner:与 S1 baseline 的日产差额,无条件渲染、永远有数字)。服务的业务目标:用多版转化文案的受控实验提升购买漏斗 L3→L4 转化率(KPI #4,§2.4.6),为主 CTA 文案提供数据驱动的版本择优——L3→L4 是用户向购买 NexGridBox 转化的关键漏斗段(§1.4)。

**② 后台界面**
四视图:文案版本配置列表(位于文案池上方)+ (a) 文案池列表 + (b) 文案内容历史 + (c) A/B 实验面板。 <!-- 冲突 #9 待裁决 -->
...
| 创建 A/B 实验排程 | 持有 `content_i1_experiment_manage` / 超管 | 标准操作确认(理由必填;仅创建排程,不对用户生效) | `admin.content_experiment_created`(实验 ID / 文案位 / 变体 / 分流 / 受众快照 / operator / reason) <!-- 冲突 #4 待裁决 --> |
...
| social-event 5 variant 概率分布 | **现状值**(§11.0A.2a:提现 30% / V 升级 25% / Genesis 成交 20% / AI 消费 15% / 小时新增 10%) | 各 0–100%,和 = 100% | 实时(对新派发生效) | social-event channel <!-- 冲突 #6 待裁决 --> |
...
| 参数 | 默认值 | 范围 | 生效时机 | 影响的前端 |
|---|---|---|---|---|
| 季度 financials 数字组(MRR / Active / Devices / Payouts + delta + footnote) | **现状值**(§11.3:Q2 2026 · MRR $4.87M +22% / Active 184,206 +38% / Devices 27,150 +12% / Payouts $47.0M +27%;取已收官季,与前端单源锚增长曲线自洽) | 受管内容(结构化数字组) | 实时(发布生效,client 拉当前发布版) | `/trust` 季度 financials section(§11.3) <!-- 冲突 #8 待裁决 --> |
...
- **跨模块联动**:I1 转化文案 / I2 推送模板 / I4 信任内容 / I5 披露文案 / I7 课程文案均引用 I6 i18n key,**I6 是全站文案的 i18n 底座**,各子模块改文案须经 I6 镜像同步;不可翻译保留英文(§14.4:品牌名 / token 符号 NEXGRID / USDT / NEX)。与 H4 边界:活动通用 copy 归 I(经 I6 i18n key),活动结构归 H4(§3.14)。 <!-- 冲突 #14 待裁决 -->
...
**① 目的 & 对齐**
管理客服中心(实现面已重组为独立域 M:`/service/tickets` 工单 · `/service/sessions` 会话;历史路径 `/content/support` 已随客服迁出域 I 退役)的 FAQ 与工单运营面,覆盖 FAQ 内容、Ticket 分类/SLA、回复、关闭、重开、owner/priority 调整与审计理由。用户端对应 `/me/help`、`/me/support`、`/me/support/tickets`。 <!-- 冲突 #11 待裁决 -->
...
3. **(c) featured / 奖励配置**:featured 位课程选择(§11.11.4:目前固定第 1 课「What is NexGrid · 5-min crash course」)+ 各课完成 NEX 奖励额配置表。
...
>
> **跨域归属(§3.14)在本章的体现**:kill-switch 状态 V1 由 A3 config store 托管(存储端权威)、**V4 本章接管管理操作面**——非 geo-block 的 5 个功能闸由 **J1** 接管、geo-block 闸由 **J2** 接管(权威随之从 A3 迁移至 J1/J2,迁移前后 B5 始终只读单一源、不持开关);篡改防御本体(server-canonical enforcement,§9.11d.2)由各业务域 endpoint 落地,**J3 是其可观测面**(只读监控,不持防御逻辑);风险披露版本切换权威归 **I5**(J4 应急剧本调用 I5 触发 re-ack,不重复持有披露权威);账户冻结态权威归 **[C2-Account]**、批量簇冻结触发归 **K1**、提现队列处置归 **D2**(J4 应急剧本编排这些域的组合动作,不重复持有其处置权)。**J 域是「统一控制面 + 应急编排」,处置的实际写入落各域权威面经各自操作确认门(确认弹窗 + 理由必填)。** <!-- 冲突 #13 待裁决 -->
>
...
| J4 | 监管点名应急 SOP | 纯运营内部应急流程编排:kill + geo-block + 披露切换 + 提现暂停 + 通知模板组合剧本 | 串联 J1 / J2 / I5 / [C2-Account] / K1 / D2 / I3(无独立前端路由) | §9.11d(应急流程) |
...
|---|---|---|---|---|---|---|
| **R1 提现激增 / 挤兑** | D 域 24h 提现申请额 ÷ 真实储备(**挤兑比率**,与 B5 挤兑预警同口径:分子 `withdraw.submitted` 24h 聚合 / 分母 B1/D3 储备) | ≥ **B5 挤兑红线 `bankrunRed`(默认 40%,B5④ 经确认弹窗可调;J1 引用不另持,引用范式同 recoverGate→B1.redLine)** | 滑动 24h | **自动熔断 + 告警** | `withdraw`(+ 可选 `exchange`,按 disposition) | 30min 内值班补录(J1-MD5) <!-- 冲突 #2 待裁决 --> |
...
- `GET /api/admin/emergency/kill-switches` — 返回 5 功能闸矩阵 `[{ key, controlledCapability, enabled, lastChangedTs, operator, triggerBasis, trigger(manual|auto), pendingAutoConfirm(bool,待补录标记), coveragePrecheckRequired, coverageImpactHint }]`，**server-canonical**（闸状态服务端权威,client 仅读状态灯渲染）。 <!-- 冲突 #5 待裁决 -->
...
**状态机**：J3 无业务状态机（纯只读监控聚合）；篡改告警 `normal → flagged(超频次阈值)→ escalated(喂 K4 + B5 后转风控处置)`——处置态在 K 域（[C2-Account]/K1）,J3 仅呈现告警态。
...
| 配置篡改告警频次阈值 / 喂 K4 开关 | 超管（当前 RBAC 尚无可验证的风控 lead 层级） | J3-MD1(理由必填;监控敏感度变更影响风控信号) | admin 审计事件（阈值 / before / after / operator / reason） <!-- 冲突 #3 待裁决 --> |
| 跳转 K（对高频篡改账户发起风控处置） | 风控（在 K 域执行处置） | （处置在 K 域经各自确认弹窗契约） | 处置审计落 K 域（[C2-Account] 冻结 / K1 批量簇） |
| 导出篡改监控报表 | 风控 / 只读审计（脱敏） | 否（只读脱敏导出,直接生效留痕） | admin 审计事件（导出范围 / operator） |

> **J3 不持处置权（核心约束）**：J3 的总览、分布和账户告警是纯只读监控——高频篡改账户的实际处置（冻结/批量簇）落 **[C2-Account]/K1**（§3.14:账户冻结权威归 [C2-Account]、批量簇触发归 K1）,在目标域经各自操作确认门(确认弹窗 + 理由必填)。J3 仅「监控 + 告警 + 跳转」,这是「驾驶舱/监控面不持处置权」在篡改防御域的体现（与 B5 风险雷达同构）。独立的监控敏感度配置不属于账户处置；当前 RBAC 没有可验证的 lead 子层级，因此只授予超管，待 A1 建立风控 lead 身份模型后再按评审结论扩权。
...
| 跳转 K 域处置 | ②(c)告警行内「去 K 域处置」 | 行内链接 | 具备目标页读取与对应动作权限时渲染;仅 flagged/escalated 态告警行显示 | 携用户编码/簇编码跳转 [C2-Account]/K1；[C2-Account] 以 `user_c2_read` 专用精确查询取得目标账户，K1 自动跨服务端分页定位目标簇；未找到或读取失败必须明示且不得默认选中其他对象。无弹窗(处置确认在目标域) |
...
**⑤ 接口**
- `GET /api/admin/emergency/tamper/overview?window=&accountPage=&accountPageSize=` — 在同一服务端时间快照内返回总览、三窗口趋势、当前窗口路径分布、按等比例阈值重算的账户告警和数据库分页（总数与页数由独立 count 查询得出，不设 200 条静默截断），另返回登记/已接线/待接线路径覆盖，以及完整的候选阈值 1–100 近 7 天回放结果，供 J3-MD1 展示服务端真实影响预览；**server-canonical**（`window ∈ {24h,7d,30d}`,非法值返回 422）。 <!-- 冲突 #5 待裁决 -->
...
> **端点收敛说明(A/B 合并标注)**:原 `GET /api/admin/tamper/paths?window=`(路径分布)与 `GET /api/admin/tamper/accounts?threshold=`(账户级告警)两端点的能力已并入 `GET /api/admin/emergency/tamper/overview` 的单次服务端快照返回(路径分布 + 账户告警 + 分页),不再单列;能力不减,仅收敛调用面。
...
- **消费（J3 是篡改拦截事件的主消费方）**：
  - `risk.tamper_detected` — **各业务域 server endpoint 拦截 client 篡改尝试时产生（emit 责任在各业务域 server endpoint;须在对应域补埋点工单,否则 J3 作为纯消费方拿不到数据）**;属性:`tamper_path（逐条枚举,见下）/ user_id / attack_effect / blocked_at_endpoint / is_server_authoritative=true`。**tamper_path 枚举(两类区分,不并)**:
    ...
    **A4 registry 已登记 `risk.tamper_detected`**；**J3 消费此事件做总览/路径/账户聚合**。11 类登记路径均已在真实服务器业务边界接线并通过拒绝分支测试；覆盖状态由发布时注册表返回 `complete`，不得用手工补数或客户端自报事件替代。 <!-- 冲突 #7 待裁决 -->
...
**① 目的 & 对齐**
J4 是**纯运营内部应急流程编排**——监管点名/法务事件触发的应急 SOP 编排(kill 哪些闸 + geo-block 哪些国家 + 披露版本切换 + 提现暂停 + 通知模板),串联 J1/J2/I5/[C2-Account]/K1/D2/I3 的组合处置剧本。对齐前端 **§9.11d**(Admin Kill Switch + Client-tamper Defense 整体应急框架——§9.11d.1 kill/geo-block 闸 + Risk-disclosure 双维 re-ack 作为监管更新条款的应急动作)。**J4 无独立前端路由**——按 `nexion-admin-prd` skill 硬规则 #5,J4 是纯运营内部应急编排能力(监管应急 SOP 是运营方内部操盘流程,用户端永不可见);其编排的每个原子动作都落各域已有的前端锚点(J1 kill / J2 geo-block / I5 披露切换 / [C2-Account] 单用户冻结 / K1 批量簇冻结 / D2 提现处置 / I3 通知)。**J4 与 J3 同为 J 域内无独立前端锚点的子模块**,Ch17 收口时无需核对 J4 独立前端路由。服务的业务目标:监管点名/法务事件发生时**一面编排全平台组合应急处置剧本**,避免人工逐域操作的遗漏/延迟,剧本步骤可预置/可审计/可追溯(§1.8 原则二/三)。

**② 后台界面**
应急剧本库 + 剧本执行面 + 执行追溯,三视图:

1. **(a) 应急剧本库**:预置应急剧本清单 `[剧本名 / 触发场景（监管点名 / 法务事件 / 突发合规）/ 编排动作序列（kill 闸集 + geo-block 国家集 + 披露版本发布 + 提现暂停范围 + 通知模板）/ 维护者 / 最近演练时刻与演练就绪态（超期未演练标「待演练」,阻断「演练就绪」）]`；剧本是「组合处置模板」,执行时按序触发各域动作。**提现暂停范围限定为「指定用户簇（→K1 批量簇冻结,联动 D2 frozen）/ 指定用户（→[C2-Account] 单用户冻结,联动 D2 frozen）」**,country 级提现屏蔽由「geo-block 关闭对应国家资金 endpoint」步（→J2）达成（非 D 域独立全局/国家提现 pause——D 域无此粒度端点,见 ③ 注 + ⑦）。
...
| **执行应急剧本（逐步确认执行）** | 风控 / 超管(发起);各原子步执行权以各域④为准(**资金步如簇/账户冻结由对应域④裁决,财务可执行该步,见各域④**) | J4-MD3(理由必填 + 触发事由;每个原子止血步仍经该域确认弹窗契约由该域授权角色单人确认执行:kill→J1 / geo-block→J2 / 披露→I5 / 冻结→[C2-Account]/K1 / 通知→I3) | `admin.emergency_playbook_executed`（剧本名 / 触发事由 / 各步动作 / 各步 operator/reason）+ 各步落各域审计 + 实时告警全体超管 + 全运营账号广播 |
| 触发披露版本发布（剧本步,调用 I5） | 风控 lead / 超管（在 I5 经其确认弹窗 I5-MD1） | （落 I5 确认门,I5④） | `admin.disclosure_published`（落 I5 审计）+ J4 编排步审计 |
| 触发账户/簇冻结（剧本步,联动 [C2-Account]/K1→D2） | 风控（在 [C2-Account]/K1 经其确认弹窗契约;**财务可执行冻结步,见各域④**） | （落 [C2-Account]/K1 确认门） | [C2-Account]/K1 冻结审计 + D2 frozen 联动 + J4 编排步审计 |
| 查看剧本库 / 执行面 / 追溯 | 风控 / 超管 / 只读审计 | 否（只读） | admin 审计事件（查看范围 / operator） |

> **J4 编排不绕过各域确认门(核心约束,回指 §15.1,仅写本模块差异)**:J4 是「组合处置编排器」——剧本的每个原子动作**仍落各域权威面经该域操作确认契约执行**（kill→J1④ / geo-block→J2④ / 披露发布→I5④ / 冻结→[C2-Account]/K1 / 通知→I3）;J4 不另立独立确认、不绕过各域确认门,也不因编排而免除任一步的 reason 强制与审计。**剧本止血序列方向 = 熔断/封锁/暂停**(组合止血步,部分失败转 partial);按 2026-06 操作确认决议各止血步为该域授权角色单人确认即时执行,执行后实时告警全体超管 + 全运营账号广播。**恢复/解封/恢复提现等放开方向不入剧本序列**——恒为各域独立动作:仅超管 + 对前置闸 B1 前置核验(422)。**各原子步角色以各域④为准**(J4 不改各域角色裁决;资金步如冻结财务可执行,见各域④,与 ⑥ 权限表对齐)。J4 是各域动作的「编排串联 + 追溯」,处置实质落各域。
...
| 逐步确认(执行中各止血步) | ②(b)执行进度每步「确认执行」 | 行内按钮 | 按该步目标域执行权渲染(无权角色只读);前序步未终态时置灰 | 打开该域对应确认弹窗(J1-MD1/MD3、J2-MD1、I5-MD1、[C2-Account]/K1/I3 各域弹窗) |
...
审计记录字段：统一 schema（§2.x A2 ⑥）`操作者(operator) / 角色 / 动作(playbook_edit|playbook_drill|playbook_execute) / 对象(剧本名 + 触发事由) / 各步动作序列 / 前值 / 后值 / 理由(reason) / 触发依据 / IP / 时间(ms) / Idempotency-Key`。**应急执行高亮审计**（监管响应取证链关键）；各原子步另落各域审计（J1/J2/I5/[C2-Account]/K1/D2/I3）,J4 追溯面交叉聚合。只读审计可全量追溯应急事件响应史。

**⑦ 风控 & 联动**
- **J4 编排串联各域,处置落各域权威面(核心分工)**:J4 是「监管应急组合剧本编排器」,串联 **J1(kill 闸)/ J2(geo-block,含 country 级提现屏蔽)/ I5(披露版本发布触发 re-ack)/ [C2-Account](单用户冻结)/ K1(批量簇冻结)/ D2(提现 frozen 联动,§9.11f)/ I3(通知模板下发,§11.2.4 critical 通道)**;每个原子动作落各域 server 权威 + 各域操作确认门（J4 不绕过——各止血步由该域授权角色单人确认即时执行,reason 强制 + 各域审计 + 告警广播）。
- **提现暂停范围与 D 域能力对齐(核心约束,臆造被控面回源处置)**:**D 域(V1)仅有 [C2-Account] 单用户冻结 + K1 批量簇冻结,§9.11f frozen 为单账户冻结标志,无全局/国家粒度提现 pause 端点**;故 J4「提现暂停」收敛为**簇级(→K1)/用户级(→[C2-Account])**（冻结确认执行后联动 D2 提现 frozen）,**country 级提现屏蔽由 geo-block 关闭对应国家资金 endpoint(→J2)达成**（非 D 域独立 pause）。若需全局/国家级提现暂停独立端点,属 D 域待补能力,记入(Ch17)。
...
- **账户/簇冻结联动资金 & 兑付**:剧本的「冻结」步经 [C2-Account]/K1 关闭对应账户/簇的资金流出（联动 D2 frozen）——方向上收紧流出（不放大）,在 B4 节奏 / B1 反映；**解冻(放开流出)经 [C2-Account]/K1 各自确认门独立执行(执行门槛按各域④就高)**,不入剧本止血序列。
...
- **消费**：J4 执行追溯面**消费各域原子步事件**（`admin.killswitch_toggled` from J1/J2 / `admin.disclosure_published` from I5 / [C2-Account]/K1 冻结事件 + D2 frozen 联动 / `nova.push_sent` from I3 通知）做交叉追溯聚合；不重复产生各域事件。
...
> 本章是全套运营控制后台 PRD(**V1–V4 四卷的原 12 域 + post-V4 K6 专项扩展 + 全局收口**,Ch1–Ch17,60+ 个 8 段功能子模块)的**收口章**。前 16 章按域纵向展开，K6 由 Janus C2-Control 专项 PRD 承载；本章横向收敛为七节并把专项登记进全局索引:① 全局数据模型总表(§17.1)② 全局 API 总表(§17.2)③ 全局横切机制总表(§17.3)④ 完整 §3.14 跨域归属总表(§17.4)⑤ 跨文档一致性最终处置(§17.5,整合附录 A)⑥ 开放 PM 决议清单(§17.6)⑦ 交付总览 + 投产就绪度(§17.7)。
>
...
| C | 用户账户 / 冻结态 / KYC 态 | C1/[C2-Account]/C4 | server | 账户冻结态权威 [C2-Account];KYC 态权威 C4;`cumulativeDepositUsdt` 仅 recordDeposit 写 |
...
| G | Genesis economics | G4 §10 | server | 节点价 $9,999(**`tiers` 阶梯档位在场时按当前档派生、`unitPriceUSDT` 只读**,PM 2026-08-06 裁定;档位表整表 `expectedTiersVersion` CAS);**dailyDividendShare=0.1%/日(PM 2026-06-01 裁定)**;二级版税 2.5% |
...
| K1–K5 | 风险评分模型 / 去重指纹 / 提现风控规则 | K4/K1/K3 | server | 风险评分(K4,B5/D2/各域消费)· IP/设备/支付三层去重(K1)· 套利/刷量检测(K2,产 risk.arbitrage_suspected)· 大额 KYC 复审(K5) |
| K6（post-V4 专项扩展） | Janus 策略、PolicyBundle、设备/命令、cohort/target catalog、接管/撤销/恢复投影 | `Nexion-janus/PRD/Nexion-Janus_C2控制后台功能设计PRD_v1.0.md` §16 | server | K6 是 Janus 接管控制域唯一人工运营写面，不替代其他业务域；C2-Control 持有策略/包/命令/accepted projection并独占策略评估，Janus 与账户服务分别产设备亲历事实和账户权威 receipt；原型只要求契约一致，不要求真实联控 |
...
| J | `GET/PUT /api/admin/emergency/kill-switches[/:key]` · `GET /api/admin/emergency/kill-switches/alerts` · `PUT /api/admin/killswitch/geo` · 篡改监控只读 · 应急 SOP 剧本 | Ch15 <!-- 冲突 #5 待裁决 --> |
| K1–K5 | `GET /api/admin/risk/users/:id/score` · 提现风控规则引擎 config · 反多账户去重簇查询 · 套利检测信号 · 大额 KYC 复审队列 | Ch8 |
| K6（post-V4 专项扩展） | `/api/admin/janus/*`：summary/devices/strategies/cohorts/remote-targets/commands/recover/audit/export、action-specific preview/grant、全protected action safe request GET、download authorization/delivery command与safe binary content GET；运行面 `/report`、`/cloak`、signed AckEvent、纯观察handoff lease GET、仅C2-Control reconciler可用的recovery/termination、closure Ack按ID/request回源、local/account recovery execution、evidence rekey/refresh、C2-Control receipt trust-set、signed account projection head/snapshot pages与连续 receipts | Janus C2-Control 专项 PRD §17 |
...
- **Janus/C2-Control 运营(K6 post-V4 扩展)**:策略、cohort、RemoteTarget与高敏动作仍按专项角色矩阵。所有production K6最终提交统一使用action-specific真实request identity与`Gateway denial+A2 | authorized domain Ack/closure+A2`，且外部响应只认`K6ProtectedEndpointResponseFor`；非grant门槛来自静态registry，grant exact requiredRole来自同一signed grant，role/capability、scope、access-revoked denial分型。确认治理由专项`K6ActionGovernanceRegistry`唯一派生：DIRECT_GATEWAY免二次确认（下载继承已确认export job理由），ACTION_PREVIEW_CONFIRMATION走普通确认，IMPACT_GRANT走高影响确认；三类均过Gateway、写唯一A2并可GET恢复。`COMMAND_RECOVER`纳入impact grant并锁recoveryTarget/operation/command、ACCOUNT_EVIDENCE时的attempt lineage与expected observation revision/hash；trigger receipt/conflict Ack不另写A2。内部恢复把早期local command/Ledger结果与account evidence checkpoint分开；rekey/refresh各有稳定request/conflict/GET与revision/generation CAS，signed evidence不可原地更新。CandidateTermination/HandoffLease/Revoke Ack完整签名，且每次projection mutation的change receipt绑定source Ack ID+canonical hash；账户所有签名事实固定issuer/audience/environment；AttemptTerminalizationReceipt由账户按fresh C2-Control receipt trust-set/head验证。K6不得直接改账户actor或伪造Janus/账户事实;

> **K6 专项横切索引**：Scope、registry、cohort/target artifact与系统Audit仍回源专项正文。R19硬门为：DesiredTakeoverIntent判别NONE/A/C/R；quota按action+channel唯一适用且安全处置不被压制；dry-run对全部eligible设备满足守恒、canonical hash唯一并用版本化assessment决定publishEligible；AckEvent安全envelope拒绝不推进水位，早期阶段禁TRANSFERRING；账户Ack/evidence签名固定audience/environment并用ID+hash互绑；projection的FINAL_EMPTY只允许signed request.after=signed head，并只返回单一`emptyFinalBoundary=head`且不复制start/last/head字段，final head只比较稳定revision/cursor/projection hash；AckGap拒绝无同attempt签名终态证明的SUPERSEDED_OR_ABSENT并给receipt/conflict专用system Audit。精确DTO、状态和错误码仍唯一回源K6专项正文。
>
> **R19 Gateway/恢复单源**：普通action门槛来自静态registry，grant exact requiredRole来自同一signed grant；最终响应只有Gateway denial+A2或authorized domain Ack/closure+A2，Gateway denial在全局A2有独立判别分支。COMMAND_RECOVER trigger/conflict由同一grant closure+A2收口，不另造A2；内部request conflict回显完整canonical request，LOCAL_COMMAND_OR_LEDGER与ACCOUNT_EVIDENCE checkpoint互斥，evidence rekey/refresh以稳定request/conflict/GET和CAS闭合。PM/demo无生产K6入口。
>
> **R18 动作/trigger占用单源**：AUTHORIZED实际角色满足exact requiredRole；CAPABILITY拒绝分支的 missingCapabilities 必须是该 action 在 exact role 下 requiredCapabilities 的非空、去重子集且不得含额外能力；Gateway外层canonical action/request identity、domain Ack discriminator与Audit gate逐项同动作。recover trigger ID由首个Gateway-authorized领域提交占用，即使原提交只有grant拒绝closure；异payload conflict引用原submission closure，原trigger不存在时不得引用trigger receipt。
>
> **K6外部响应、恢复与账户receipt单源**：direct-write Ack/Audit绑定真实request/canonical ID、fingerprint、actor/session/scope、Gateway receipt/fence/binding与唯一A2；challenge CONSUMED只引用首胜submission closure。recover trigger/result/checkpoint共享target+action并做observation CAS，漂移只返回无evidence的RECOVERY_OBSERVATION_CHANGED；AckGap穷尽全部A/C恢复态，pending/finalizing不启动timeout。subject projection无顶层单binding，active actor与唯一`pendingCandidate`分别自带binding；handoff公开DTO显式区分A/C。reservation commit和ledger append均为可按ID回查的账户域分离签名receipt，append绑定commit ID/hash。所有user-facing owner/nextAction都必须对应resolved route。精确DTO仍唯一回源K6专项正文，本卷不形成第二套接口。
>
> **R21 K6完整性索引**：同target事实推进与跨target切换都由recovery observation CAS显式返回，均无evidence并重开preview；全部protected action注册真实request字段→write method/path→safe GET→typed response，PUT/DELETE不得伪装成POST。§17全部production method/path/visibility行与endpoint manifest双向精确相等，所有实际HTTP响应（列表、详情、嵌套history、内部read model）统一递归解析typed route，未知pair/raw string route/未登记端点fail-closed。challenge POST/GET只签发/恢复；下载为POST授权、POST交付命令、sealed secret、principal/session/scope绑定content session及无mutation/A2的safe binary GET。所有非Rule number路径进入typed range/scale/precision registry与binary64 closest-then-even边界向量。command只在commit→Ledger→append receipt→同一C2-Control事务完成A/C INITIAL slot或REVOKE双checkpoint物化后accepted，未终态/失败分别返回PROCESSING/CREATION_FAILED同态结果。AckGap贯穿同一gap/action/device/generation/command/attempt/hash tuple并先tuple CAS；closure Ack与delivery receipt分别使用账户/C2-Control域签名、绑定terminalization/commit/append及子Ack ID+hash并按fresh lifecycle回源验真。Janus/远程H5只做lease GET观察，不得调用内部recover。精确DTO仍唯一回源K6专项正文，本卷不形成第二套接口，也不表示实现或真实联控。
>
> **R22 K6索引**：§17叙事组原子展开method/path/caller并绑定具体request/response schema及variant；protected action引用实际DTO/path/confirmation/current auth，stable GET重验当前授权，route完整entry只有一份。恢复target权威单调，ACCOUNT→LOCAL为fail-closed事故；非终态不得走SUPERSEDED_OR_ABSENT，CHANGE_TARGET替换actor后的旧rebind有专用拒绝。AckGap timeout绑定当前gap/expiry/tuple/sequence/expiry hash；全部canonical number path与三类registry精确相等；binary GET每次重验current capability/access/scope。K6六区、列表/详情、MD1～11统一六态和安全退出；CommandCreationCheckpoint不含外层LedgerReconcilingRecovery。本文仍只作全局索引，精确合同回源专项PRD。
- **kill-switch**:6 闸(5 个二元功能闸 + geo-block;V1 入口在 A3、V4 归 J1/J2;**恢复方向(disable→enable)执行=仅超管 + B1 红线前置,熔断方向为止血动作,执行=风控/财务/超管**);
- **账户高敏处置**:账户冻结 / 解冻([C2-Account])、impersonate 授权([C2-Account])、人工标记/撤销 KYC(C4,执行=风控 lead/超管)、人工 disable 2FA / 密码重置 / 解除账户锁定(C5);
- **批量账户簇冻结**:批量冻结关联账户簇(K1,执行=风控 lead/超管;单用户冻结→[C2-Account]);
...
| K6 策略/PolicyBundle/命令 Ledger/accepted 设备投影 | C2-Control | K6 人工运营与只读观察；command accepted还要求append receipt与action-specific物化同一事务完成；Janus只消费已物化命令/cleanup slot并回Ack/Report，`bundleVersion`仅作来源元数据，不接收或评估权威规则树。Janus禁止直接调用账户handoff写、abort或C2-Control内部recover；唯一例外是Janus/远程H5持完整Grant/fence对`GET /handoffs/:handoffId/lease`做safe纯观察，且不得产生账户mutation。Ack精确重放返回首次持久化disposition |
| Janus 设备亲历阶段事实 | Janus 安装主体签名事实 | C2-Control 验签、排序、收敛后投影给 K6；K6 不得自造 |
| 账户 active actor/candidate/lease/revoke/change receipt | 账户服务 | 签名 receipt 携 changeCursor/previousCursor/current hash；C2-Control 验签并连续摄取后形成 accepted cursor/revision/head-hash projection；K6 只读脱敏状态与恢复 owner |
| K6 cohort/RemoteTarget 当前可用性 catalog | C2-Control | immutable 内容 artifact 与 catalog availability/producer provenance 分离；RemoteTarget artifact 显式携 contentHash；旧 bundle 保留历史快照 |
| KYC 态 / jurisdiction | C4 | D2/G2/K5/I5(disclosure 输入) |
| 账户冻结态 | [C2-Account] | D2(提现)/ K1(批量簇冻结触发)/ J4(应急编排) |
...
- 运营后台需求正文是 `NexGrid_运营控制后台PRD_v1.md`～`v4.md` 四卷按域共同组成，V4 第17章只是全局索引/收口，不能替代 V1～V3 正文。 <!-- 冲突 #15 待裁决 -->
- `D:\WORKS\PLAN\PRD\NexGrid_运营控制后台_开发落地规格.md` 是四卷与专项 PRD 的开发契约镜像，不覆盖上游业务语义。
- K6 post-V4 专项正文固定为 `D:\WORKS\PLAN\Nexion-janus\PRD\Nexion-Janus_C2控制后台功能设计PRD_v1.0.md`；K6/C2-Control/Janus/账户接管主题冲突时以该专项正文为主题权威，并同步回本章索引与开发规格。
...
**交付物分层**：V1～V4 四卷是 pre-K6 后台主卷；K6 post-V4 的跨端业务合同唯一回源专项正文 `Nexion-janus/PRD/Nexion-Janus_C2控制后台功能设计PRD_v1.0.md`。后台开发规格和映射文档只保留本端 IA、RBAC、入口与关键不变量，不复制第二份 endpoint/route/schema 合同。K6 主题冲突只回源专项正文。 <!-- 冲突 #16 待裁决 -->

**K6/Janus 业务基线**：专项正文独占接管策略、命令、设备事实、账户 handoff、失败和恢复合同。账户 handoff 使用当前版本的完整快照、隐藏暂存、lease CAS 发布、唯一 pendingCandidate 和连续权威 receipt；历史协议只供识别，不得进入当前写链。R24–R26 的生成表、hash、validator 和 reviewer 批次已退役，不是产品合同，也不表示代码或真实联控完成。
...
**pre-K6 四卷就绪度与 K6 NO-GO**:

- **✅ pre-K6 文档结构完整**:12 域 + 全局,~69 个 8 段功能子模块 + 3 个结构索引节(§15.1/§16.1/Ch17),每子模块可追溯前端 §锚点 + 业务目标 + 数据 + 接口 + 状态机。该结论不覆盖 K6/C2-Control/Janus/账户接管专项。
- **🔴 K6 NO-GO**：SPEC00 尚未由主线及至少三路独立审查共同得到 0 P0 / 0 P1 / 0 P2 前，K6 只能视为原型设计审查中；不得宣称专项定稿、可实现、可投产或真实联控。后续 SPEC01～SPEC07 与全局双向总审也必须全部关闭。
