...
**已在本轮清掉(提交 `e5a26af`)**:m3 那条守死代码的断言(已改钉现行中文判据,16/16)· c1/d5 死分支
(文案挪进映射表单一真源;**顺带证伪了「c1 那条已死」的审计判断**——该码是组件本地抛出、不经 client 翻译,
提示一直是活的)· h-client 三处字段名上屏(改机器码+补表,字段名只留日志)· h-client 的 `response.json()`
补 `.catch(() => null)` 与兄弟 client 对齐。补表条目已**机器核**:21 条逐条验「代码里真有产地」+
「显示时命中自己不被同族前缀抢占」,结果 0 抢占。

**仍未做**:
- **T6b(K/L/M 展示面)的独立验收未完成**——其 agent 被会话额度杀在中途。该范围现由哨兵四判据机器覆盖
  (全仓 291 文件不分域),文案口径仍缺人核一遍。
- A2 提案失败会弹两条 toast(`propose-or-execute.ts` 一条 + `e-view` 一条),消重涉及业务流程,需产品决定。
- **body 读取阶段仍在咽喉之外**(§4.1):`res.text()`/`res.blob()` 中途断连抛的英文 TypeError 不经 guardedFetch。
  现有缓解:凡走 displayAdminError 的展示面已被构造性兜底(无中文→中性文案)接住;未走展示边界的仍留缝。
- 多域实景断网矩阵 + nexion-audit(缺 nexion-backend 进不去内页,见上)。
