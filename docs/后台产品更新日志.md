...
### ✏️ A1 强制登出权限收口 · Redis 会话真实吊销

**概述**：A1 运营账号“强制登出”改为服务端强校验:账号不能登出自己;只有超管或风控可以操作;目标账号不能是超管。后端通过登录 JWT 的管理员 ID 定位当前操作人,通过 Redis 后台会话索引执行真实吊销,前端只做按钮禁用和原因提示。

**要点**：
- 后端 `revokeSessions` 在调用 Redis revoke 前校验当前账号、发起角色和目标角色,拒绝原因返回中文可读错误码。
- 前端 A1 列表按当前登录账号与 A1 overview 的有效角色禁用按钮,无活跃 Redis 会话时提示“目标未登录”。
- PRD 接口表和权限矩阵同步为“超管/风控 + 非自己 + 非超管目标”。

**入口**：/platform/rbac（A1 管理员与 RBAC）｜ **PRD**：✅ 已同步——A1-MD6、接口清单和角色权限矩阵。

---

### ✏️ A1 管理员账号层级移除 · 执行门槛改为角色授权 / 超管

**概述**：A1 管理员账号不再区分旧双层级。账号创建、改角色、A2 操作确认、I/J/K/L 高敏动作说明和后端种子统一按“角色授权或超管”展示与校验；M 域客服岗位继续由客服中心配置，不再由 A1 账号层级派生。

**要点**：
- A1 创建账号、改角色接口与前端表单移除层级字段，只保留结构化角色选择。
- A2 审计中心、内容披露、风险/应急/导出相关执行门槛文案统一改成角色 / 超管口径。
- 客服岗位与专属顾问仍在 M 域配置，A1 只负责后台管理员身份与基础角色。

**入口**：/platform/rbac（A1 管理员与 RBAC）｜ **PRD**：✅ 已同步——RBAC 弹窗规格、A1 OperatorAccount 数据模型、操作确认迁移规则与权限矩阵已移除旧双层级。
