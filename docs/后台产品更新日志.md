...
- **启动边界**：后端和 5173 标准开发启动脚本不再默认注入 `nexion-local-dev` Acceptance RunID；专项 `test`/验收 Sandbox 仍需显式启动，不会被普通开发流程误开；
...
**区别说明**：此处 `PRODUCTION` 表示“正式业务数据轨”，不代表开发环境连接线上生产数据库。`dev` 与线上生产由运行环境及数据库实例隔离；Sandbox 是另一条必须显式开启的专项验收轨。
