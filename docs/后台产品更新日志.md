...
### ✏️ E5「设备库存 & 激活」接入真实设备接口

**概述**：E5 设备库存表从前端 `FLEET` 样本和 `E.device.*` 本地状态切到后端设备运维接口，激活、取消激活、解绑和 DC pause/resume 均通过 Next 代理转发到 Spring Boot。

- **✏️ 前端**：新增 E5 client，列表读取 `GET /api/admin/devices`，统计读取 `GET /api/admin/devices/overview`；表格展示 loading / error / empty，不再静默显示 mock 设备。
- **✏️ 展示修正**：设备库存表新增服务端分页组件；设备列改展示 `instanceNo`，设备名称单独成列；用户列展示后端 `nickname + userNo`；SKU 列仅展示产品编码 / 档位，避免与设备名称冲突。
- **✏️ 操作**：激活/强制激活走设备恢复接口，取消激活/解绑走设备停用接口，DC pause/resume 走后端数据中心处置接口；全部带理由、操作人和 `Idempotency-Key`。
- **入口**：设备与商城 → 设备运维 → 设备库存 & 激活。
- **PRD 章节**：E5 设备运维。
