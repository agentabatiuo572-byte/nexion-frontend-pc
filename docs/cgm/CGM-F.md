...
| CGM-F-020 | platform | param-config | welcomeGift.usdtAmount=5 / nexAmount=20 (注册 … | fund_safety,conversion,phase_12mo,risk | 运营在 K2「新人礼发放配置」调金额/模式(已落地·操作确认+审计;升额=放大获客成本流出过 B1) | Y | GET /api/config/sponsorship (读) · PUT /api/admin/… | gap |
| CGM-F-021 | platform | param-config | otpGate.resendSeconds=60 / captchaAfterSends=2 / otpTtlSeconds=300 / maxVerifyAttempts=5 / captchaTicketTtlSeconds=120 (短信验证码闸门,uniapp otpGate 同键) | risk,platform_integrity,compliance | 运营在 K2「短信闸门参数」调 5 个数值(已落地·操作确认+审计;放宽=短信轰炸/批量收码面变大,收紧防误伤) | Y | GET /api/config/platform (读·otpGate 组) · PATCH /api/admin/otp-gate/config | gap |
