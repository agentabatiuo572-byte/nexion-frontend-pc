...
- **config archetype**:参数 = 受控输入 + 脏检测 + 重置 + 应用变更(总管理员仍需操作确认即时生效 / 其余 操作确认)。
...
| A2 审计&操作确认 | 敏感操作确认 | 待确认行 通过/驳回(操作确认) | list 默认操作确认 |
...
| 模块 | 前端杠杆 | 后台控制 | 类型 |
|---|---|---|---|
| D1 充值对账 | 逐笔对账 | 核销差异/挂账/标记确认 | list rowActions |
| D2 提现审核队列 | 提现放行 | bespoke(放行/延迟/冻结+红线) | 旗舰 |
| D3 资金池仪表盘 | 水位 | controlLink→/finance/params | dashboard |
| D4 账本审计 | 双账本流水 | 导出凭证/标记确认 | list rowActions |
| D5 提现参数 | 日限/费率/冷却 | 可编辑参数 + 提交(红线核验) | 旗舰 config |

### E 设备与商城
| 模块 | 前端杠杆 | 后台控制 | 类型 |
|---|---|---|---|
| E1 商品目录&代际门 | SKU/价/库存 + 代际门 | 新增 SKU + 上架/下架/改价 + 代际发布门 P1-P6/强制解锁 | list rowActions + timeline |
| E2 收益&任务引擎 | baseRate/任务 | 任务改单价 + 新增/下架(真 store) | config |
| E3 生命周期&Trade-in | 衰减/折抵率/持有期 | 三段衰减调参 + 折抵率/minHolding/promo(操作确认) | config + 曲线 hero |
| E4 订单状态机 | 订单流转 | 退款/取消/补建终态 | list rowActions |
| E5 设备运维 | 健康/工单 | DC 批量 pause/恢复 + 健康详情 | list rowActions |
...
| F4 池 / 配额 / 大使 / 榜 | 池比例/配额门槛/大使确认/榜池(聚合原 F6/F7/F8 → F4b/F4c/F4d) | 可编辑配置 + 处置(操作确认) | config + list/dashboard 聚合 |
...
| list 行操作(A1/A2/C2/D1/E1/I4/K1) | 行详情抽屉出操作按钮 ✓(K1=确认关联/解除/封禁全簇;A2=确认通过/驳回;C2=解冻 等,按真实状态门控) |
...
> **控制机制随设计稿改变**:由「archetype 可编辑输入 + 应用栏」改为设计稿的「**动作按钮(定价/调整/调价/受理/确认/pause)→ 操作确认弹窗** + 开关 Toggle(kill-switch/staking 各档)+ 新增入口(新增 SKU 抽屉,拖拽+1:1 裁剪)」。每域控制面仍齐备且角色感知(总管理员仍需操作确认),只是交互形态对齐设计稿。
...
> **逐条补齐**(7 agent 并行,中性语言,复用各视图 操作确认 + useToast):
> - **config 型**(A3 系统配置 / G2 兑换阈值 / G3 做市参数 / H2-H6 试用·Quest·签到·里程碑 / K3 提现规则 / K4 评分权重):每参数行加「调整」按钮 → 操作确认(放大流出项 amplifies=true 触发 B1 覆盖率核验)。
> - **list/确认 型**(A1 账号·A4 事件·F4 大使·I1 文案·I3 Campaign·I4 信任·I5 披露·I6 i18n·I7 教程·J3 篡改/SOP·K1 关联簇):行级操作(停用/改角色/确认/封禁/发布/下架/设为胜出/确认拦截/启动演练…)+ 新增入口(新增账号/事件/Campaign/教程,部分带 Drawer)→ 操作确认。
> - **F1 V-Rank**:门槛调整 + 实物发货队列(培育奖 NEX amplifies)。
> **复审:0 零控制 Tab** — 全 12 域、每个 Tab 均有 ≥1 可操作控制面(角色感知,总管理员仍需操作确认)。功能抽验 E1 定价→操作确认弹窗 ✓。tsc 0 · verify 114/0 · 浏览器自检 0 报错。
