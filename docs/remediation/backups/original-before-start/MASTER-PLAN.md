...
> **任务状态：⏸ 暂存待启动**（2026-06-12 主人指令）
> **启动条件**：主人完成剩余域的新设计稿替换后，由主人下令启动本任务。
> **启动方式**：新会话对 Claude 说「启动整改任务 / 按 MASTER-PLAN 开始」，先读本文件再干活。
> **启动后第一步**：M0——建文档体系骨架（specs/_TEMPLATE 已就位）+ L0 五张全集清单 + 台账 schema 校验脚本，然后启动 L1 第一轮五源发现。
...
| 里程碑 | 内容 | 状态 | 需要主人的决策 |
|---|---|---|---|
| **前置** | 主人完成剩余域新设计稿替换 | ⏳ 进行中（任务启动条件） | 完成后下令启动 |
| **M0** | 文档体系落库 + L0 五张全集清单 + 台账 schema | 文档体系 ✅（2026-06-12）；其余待启动 | — |
