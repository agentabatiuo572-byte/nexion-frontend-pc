...
> **当前进度**：2026-06-26 后当前整改/audit 口径已切到 Admin + UniApp 两端。L1 分片计划现为 UniApp 81 路由 + Admin 66 路由，旧 H5 只保留历史 evidence/spec，不再进入新分片、runtime sampler、feature mapping 或 owner live gate。
...
**范围与前端定位**：
- 前端**唯一实现面**——`Nexion-uniapp`（Vue3，dev:h5 → 5173）是前端交付主体。
- 旧 H5 工程已于 2026-06-26 退役删除；不再作为迁移源、审计 cross-reference 或工作线。
