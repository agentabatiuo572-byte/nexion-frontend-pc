...
- **双币体系**:收益与结算采用双币——**USDT**(出金、设备结算、提现的稳定币)与 **NEX**(平台代币,用于锁仓 / 兑换 / 二级市场)。后台所有金额参数须明确所属币种。
...
| 团队分润佣金体系 | 影响力网络版税(Unilevel)+ 双轨对碰 | Unilevel 费率(L1-L7 + Rate Tier)、V-Rank 门槛、binary 双轨日封顶(`binaryDailyCapUSD`,Phase 派发,§13.4.1)、提现冷却、领导池 |
| 代币经济 | NEX 平台代币经济(锁仓 / 兑换 / 二级市场) | NEX 周价格曲线关键帧排程与做市、Staking 池(USDT 锁仓 + NEX 池)APY/penalty、Genesis 节点经济与二级市场、Exchange 三阈值 |
...
> **Staking 收入科目澄清**:Staking 的 APY/penalty 是**用户侧产品参数**(USDT 锁仓 + NEX 池两个独立产品,§13.3.1 / §9.6),归入"代币经济"支柱作为资金留存与 NEX 需求杠杆,**不单独列为一条收入来源**;前端 PRD §1.4 未将其列为独立收入科目。
...
**参数默认值以 `NEXION_12月节奏表.md` §6.4 `ProductPhase` 为权威**(2026-06-01 directive,优先于前端现状)。该 12 月运营模型**按月**派发 **7 个时变 dial**(新用户加成 / 邀请加成 / 复投加成 / 提现 NEX 闸 / 提现冷却 / 双轨日封顶 / 任务加成);后台再叠加前端 §13.4.1 的 **`complianceHoldEnabled`**,合计 **8 dial**。下表按 P1–P6 命名带汇总(部分 dial 在带**内**按月有拐点,见表注),完整逐月矩阵在 **Ch7(H1)** 展开。

| Phase | 月段 | 名称 | 新用户加成 | 邀请加成 | 复投加成 | 提现 NEX 闸/$100 | 提现冷却 | 双轨日封顶 | 任务加成 | Compliance hold |
|---|---|---|---|---|---|---|---|---|---|---|
| P1 | 0–2 | 极速拉新 | 2.0× | 2.0× | 1× | 10 | 30d | $5,000 | **4×** | — |
| P2 | 2–4 | 温和过渡 | 1.5× | 1.5× | 1× | 10 | 30d | $5,000 | 1× | — |
| P3 | 4–6 | 首波升级 | 1.0× | 1.0× | **2×**(月5–6限时) | 10 | 30d | $5,000 | 1× | — |
| P4 | 6–8 | 订阅化 | 1.0× | 1.0× | 1× | 10 | 30d→**35d**(月8) | **$2,000**(月7起) | 1× | — |
| P5 | 8–10 | 沉淀加深 | 1.0× | 1.0× | 1× | 10→**20**(月9) | 35d→**45d**(月9) | $2,000 | 1× | **✅** |
| P6 | 10–12 | 软退场 | 1.0× | 1.0× | 1× | 20 | 45d | $2,000 | 1× | ✅ |

> **加粗 = 该 dial 在此带内的变化拐点。** 12 月模型是**月粒度**的,比 6 个等宽 Phase 更细:双轨日封顶在**月 7**(P4 带内)由 $5,000 降 $2,000;提现冷却**月 8** 升 35d、**月 9** 升 45d;提现 NEX 闸**月 9** 由 10 升 20;复投加成仅**月 5–6** 限时 2×。后台 H1 调度器以**月粒度**为准,P1–P6 仅为命名带。
>
> **默认值权威 = 12 月周期,前端为现状参考**:上表默认值取自 `NEXION_12月节奏表.md` §6.4 `ProductPhase`。前端 §13.4.1 是当前原型的**简化实现**(仅 邀请加成 / 提现 NEX 闸 / 提现冷却 / 双轨日封顶 / Compliance hold,按等宽 Phase 取值、**无月内拐点**),且**缺** 新用户加成 / 复投加成 / 任务加成 三个倍率。冲突时**以 12 月周期为准**;前端现状差异(如双轨封顶拐点:§13.4.1 标 P4=月6,12 月模型为月7;前端无 35d 中间档;前端任务加成实现为 1.0→1.5 ramp 而 12 月模型为月1–2=4×、其余=1×),开发对照现状↔目标实现。
>
> **dial 在 V1 的落地边界**:H1(★V1)提供全 **8 dial** 的**查看 / 调优 / 按 cohort 下发**;但各 dial 的**生效逻辑**落在其业务域——提现 NEX 闸/冷却在 D 域(V1)、双轨日封顶在 F 域(V2)、邀请/新用户/复投/任务加成在 H3–H6(V3)。即 V1 可调可下发**全部** dial 值,部分 dial 的对账与可视化随对应域批次补齐。
...
- **布局结构**:1. **信息区**:目标账号 ID / 显示名 / 2FA 绑定时间 / 最近登录 IP 与时间(防社工核对)。2. **输入区**:reason(多行文本,必填,8–200 字)+ 确认勾选「我已通过独立渠道核实本人身份」(必勾)。3. **按钮区**:取消 / 确认重置。
...
| D5 | 提现参数配置 | 日限/80%上限/fee/cooldown/NEX 闸(Phase 联动) | §9.3.2 / §13.4.1 | V1 |
...
| F2 | 网络版税费率 | /team/unilevel(L1-L7 USDT+NEX)+ Rate Tier | §8.3 / §9.11c.1 | V2 |
...
| 子模块 | 名称 | 对应前端 | §锚点 | 批次 |
|---|---|---|---|---|
| G1 | Staking 池配置 | /staking(USDT 4 档 + NEX 4 档)/ penalty / kill | §9.6 / §9.11c.1 / §9.11d.1 | V3 |
...
| 跨域能力 | 权威归属 | 引用方 |
|---|---|---|
| Phase 引擎参数表(**8 dial**,月粒度;§1.7 权威) | **H1** Phase 调度器 | B4 节奏状态(只读展示)· D5 提现参数(cooldown/withdrawNexGate 随 Phase)· F3 双轨(binaryDailyCapUSD 随 Phase) |
...
- **跌破红线联动**:覆盖率 < 红线 → ① B1 告警条置顶 + ② **联动 B5 风险雷达**点亮覆盖率告警灯 + ③ 系统**建议收紧提现 dial**(提现 NEX 闸上调 / 冷却延长 / 日限下调,对应 H1 dial + D5 提现参数),**仅生成建议,不自动执行**任何 dial 变更或 kill-switch(避免误触发挤兑)。实际收紧由运营在 H1/D5 经确认弹窗(理由必填)执行。
...
2. **8 类应付负债科目分解**(堆叠条 + 明细表,各科目占比 + 绝对额 USDT):
   ...
   | 8 | 锁仓本息其他 | 上述未覆盖的锁仓类应付本息(预留科目,口径与 D3 对齐) | 相关 `staking.*` 派生 | §9.6 |
...
2. **8-dial 现值表**(引用 §1.7,只读):
   ...
   | 提现 NEX 闸 / $100 | 〃(月 9 由 10 升 20) | §6.4 `withdrawNexGate` | D 域 V1 已生效 |
   ...
   - **「V1 生效状态」列说明**:H1(★V1)对全 8 dial 提供查看 / 调优 / 按 cohort 下发,但各 dial 的**生效逻辑**落在其业务域(§1.7"dial 在 V1 的落地边界")。该列标注每个 dial 在 V1 阶段是否已在对应业务域落地生效,防止运营误判"全部 8 dial V1 均可操作并即时生效"——V1 可调可下发全部 dial 值,但仅 D 域相关 dial(提现 NEX 闸 / 冷却 / Compliance hold)在 V1 已实际生效,其余随对应域批次(F=V2、G/H3–H6=V3)补齐。
...
> **P5–P6 高摩擦阶段挤兑阈值的 Phase 联动建议**:P5–P6 阶段提现冷却达 45d、NEX 闸翻倍,24h 提现申请量自然受压。建议 P5 起下调黄线至 10%–15%、红线至 25%–30%,以维持雷达预警灵敏度。H1 Phase 调度时同步更新 B5 挤兑阈值为**连带操作项**(Phase 联动调整建议值);运营在 P5 拐点须将该阈值调整列入 H1 调度的连带 checklist。
...
|---|---|---|---|---|
| 兑付覆盖率跌破健康线 | ⑤ 覆盖率告警 | B1 / `admin.coverage_threshold_breached`(§2.4.5 ⑥) | **B1** 双账本(核验)→ **H1/D5**(收紧 dial) | 核验覆盖率口径;系统**建议**收紧提现 dial(提现 NEX 闸上调 / 冷却延长 / 日限下调),实际收紧经 H1/D5 确认弹窗(理由必填)执行,B5/B1 不自动执行(§[B1]⑦ 跌破红线联动) |
...
> - **server 唯一账本 → D4 权威**:C3 USDT / NEX 余额调整**写 D4 账本**(D4 是 server 唯一记账源,§9.11d.2),C3 不另立账本,调整即在 D4 落 bill;积分(`points`)为独立 store(§12.12),仅 C1 只读展示贡献积分、已非提现门槛(门槛改 NEX 燃烧,见 D5/H1),C3 不再调整积分。
...
- **布局结构**:1. **信息区**:userId(链 C1)/ 当前 usdtBalance / nexBalance / 来源请求单号(从请求单进入时)。2. **影响预览区**(credit 方向必有):server 预检「调整后兑付覆盖率」,低于黄线警示、低于红线阻断(确认钮置灰,文案含「server 将拒绝(422)」);金额 > $500 时提示行「超额路径:执行须财务(lead)/超管资质」。3. **输入区**:见下表。4. **按钮区**:取消 / 确认调整。
- **输入与选择控件**:
...
- **跨模块联动**:USDT / NEX 调整产 `admin.balance_adjusted`(发起层)+ `admin.bill_adjusted`(记账层)喂 D4 账本 + **D3 水位**(资产调整影响负债账本 → 喂 D3 储备 / 负债聚合 → 影响 B1 覆盖率);NEX 调整另影响 D2 提现「NEX 是否足烧」判定——C3 调整 NEX 余额后,**D2 队列在下次提现校验时由 server 读取最新 NEX 余额**(server-canonical),「NEX 是否足烧」= 当前 NEX 余额 ≥ `ceil(withdrawUsdt / 100) × withdrawNexGate`(每 $100 燃烧 N NEX,`withdrawNexGate`,§9.3.2 公式;D5 / H1 派发)。
...
| 登录失败锁定(短期) | **5 次 / 15 分钟**(§4.6.2) | 3–10 次 / 5–60min | 实时 | §4.6.2:密码 / 2FA 连错 5 次 → 15min 锁定;**前端现状参考** |
| 登录失败锁定(长期) | **10 次 / 24 小时**(§4.6.2) | 5–20 次 / 12–48h | 实时 | §4.6.2:密码连错 10 次 / 24h → 24h 锁定 + 强制 reset;**前端现状参考** |
...
> - **C3⑦ NEX 闸实时重判机制**:C3 调整 NEX 余额后对 D2 在途申请触发提现 NEX 闸门槛重判的机制是否在 D2 侧 §9.3.2 定义,须回源 D2 确认;若 D2 无对应 endpoint 契约,则退化为「下次提现请求由 server 实时读 NEX 余额」、不承诺在途重判。
...
>
> 其余跨域归属:**② 提现 cooldown / NEX 闸** 是 Phase 派发参数,权威归 **H1**,**D5 是生效面而非权威源**(不另立两源);**③ 风险评分** 权威归 **K4**(D2 消费,作提现路由依据);**④ 审计 / 操作确认** 权威归 **A2**;**⑤ 埋点命名 / 身份 / 通用属性** 权威归 **A4**。D 域不重复定义这些权威源,只作生效 / 消费 / 产生方。
...
**(a) 队列列表**:`[withdrawalId / userId / 金额(USDT)/ 目标地址 + 链(TRC20/ERC20/BTC/ETH)/ 提交时间 / 风险评分(来自 K4)/ KYC 态(来自 C4)/ NEX 是否足烧(D5 派发的 withdrawNexGate 门槛)/ 24h 第几笔 / 命中规则(来自 K3 提现风控规则引擎)/ 当前状态]`;支持按状态 / 风险评分 / 金额 / 命中规则筛选与排序。风险评分**引用 K4**(§3.14:风险评分权威归 K4),D2 不另设评分模型;KYC 态**引用 C4**(§3.14:KYC 状态权威归 C4)。
...
> **默认值口径**:SLA / 自动路由阈值锚 12 月运营周期兑付节奏 + 前端 §2.3 提现 SLA;**cooldown / NEX 闸门槛不在 D2 设定**——其为 Phase 派发参数,权威归 H1、生效面在 D5(§3.14;见 D5)。D2 消费 D5 生效的门槛值作为"NEX 是否足烧"判定,不重设。
>
...
- **布局结构**:1. **信息区**:withdrawalId / userId / 金额 / 当前状态 / 当期 Phase 合规审查窗口上限。2. **影响预览区**:提示行「延迟收紧流出,不触发 refund;到期自动回审核队列」。3. **输入区**:持有天数(数字输入,1–45d,≤ ③ 表 delay 时长上限,默认对齐 Compliance hold 窗口)+ reason(必填,8–200 字)。4. **按钮区**:`[取消]` · `[确认延迟]`(置灰条件 / loading;携 `Idempotency-Key`)。
- **错误态**:409 / 400 `REASON_REQUIRED` / 422(天数超窗口上限)/ 403。
- **成功反馈**:关窗;行内状态更新 extended-hold(含到期日);toast;事件 `withdraw.delayed`(holdDays / reason)。

##### [D2-MD4] 提现资金冻结确认
- **功能**:对在途提现执行 freeze(转 `frozen` 态,账户资金冻结),确认即生效。
- **布局结构**:1. **信息区**:withdrawalId / userId(链 C1)/ 金额 / 当前状态 / K4 风险评分与命中证据 / 关联 C2 账户冻结态(如有)。2. **影响预览区**:提示行「冻结后该单停在 frozen 态,解冻回 review-pending;账户级冻结另走 C2(本动作仅冻结本单资金)」。3. **输入区**:reason(必填,8–200 字)。4. **按钮区**:`[取消]` · `[确认冻结]`(警示色;置灰条件 / loading;携 `Idempotency-Key`)。
- **错误态**:409(非在途态)/ 400 `REASON_REQUIRED` / 403。
- **成功反馈**:关窗;行内状态更新 `frozen`;toast;事件 `withdraw.frozen`(reason / operator);实时告警风控 lead;喂 B5。
...
**⑤ 接口**
- `GET /api/admin/withdrawals?status=submitted|review-pending|review-passed|processing|sent|confirmed|review-rejected|address-invalid|tx-failed|tx-orphaned|refunded|frozen&cursor=` — 队列列表(按状态筛,游标分页),返回列表字段含 `riskScore`(K4)、`kycStatus`(C4)、`nexGateOk`、`count24h`、`hitRules`(K3)。
...
- **cooldown / NEX 闸门槛来自 D5(Phase 派发)**:提现 `withdrawCooldownDays` 与 `withdrawNexGate` 门槛由 H1 经 Phase 派发、D5 为生效面(§3.14),D2 队列消费该门槛判定"NEX 是否足烧 / 冷却是否到期",**不在 D2 重设**。
...
**② 后台界面**
五区:真实储备明细 + 8 类负债科目分解 + 到期负债预测(7/30 天)+ 净敞口曲线 + 对账导出。
...
2. **8 类应付负债科目分解**(堆叠条 + 明细表,各科目绝对额 USDT + 占比;科目明细作为 B1 负债账本的聚合输入,口径与 B2 科目**严格一致、单一源**,编号完全镜像 B2 -640):
   ...
   | 8 | 锁仓本息其他(预留) | 其他锁仓本息应付的预留科目(扩展位) | 预留(按需启用,扩展走 §2.4.8 schema 治理) | §9.6 / 扩展位 |
...
- **布局结构**:1. **信息区**:billId / userId(链 C1)/ type / 原金额与状态 / 关联 ref(order / withdrawal / exchange ID)/ 核查结论引用(从异常核查进入时)。2. **影响预览区**:server 预检调整后该用户 Running Balance(断点是否闭合);**credit / refund 方向必有**「调整后兑付覆盖率」预检——低于黄线警示(当前值 + 黄/红线),低于红线阻断(确认钮置灰,文案含「覆盖率低于红线,server 将拒绝(422)」)。3. **输入区**:调整类型(下拉单选:手动 refund / 金额更正 / 状态更正)+ 目标新值(数字 / 状态控件,按调整类型渲染)+ reason(多行文本,必填,8–200 字;server 空值 400 `REASON_REQUIRED`)+ evidenceRef(关联凭证,必填)。4. **按钮区**:`[取消]` · `[确认调整]`(警示色;必填未过 / 覆盖率阻断置灰;loading;携 `Idempotency-Key`)。
...
**① 目的 & 对齐**
D5 是**提现摩擦的运营杠杆**生效面——五类提现参数的后台展示(其中日限额 / 余额上限 / 网络费三类由 D5 配置,cooldown / NEX 闸由 H1 Phase 派发 D5 只读)。对齐前端 §9.3.2(提现燃烧 NEX 门槛)+ §13.4.1(Phase 派发提现参数)+ §9.11c.1(Wallet 提现限额 + fee 公式 + 日额度)+ 12 月 §6.4。服务的业务目标:提现摩擦的运营调节(日限 / 上限 / fee 由 D5 owns),同时**对齐 Phase 派发**——**cooldown / NEX 闸是 Phase 派发参数,权威归 H1,D5 是生效面**(§3.14),D5 不独立设这两个参数,避免与 H1 双源。

> **D 域生效面定位(§3.14 铁律)**:提现 cooldown / NEX 闸由 H1 Phase 引擎按用户 Phase 下发(权威源 H1),**D5 是这两个参数在 D 域的生效面 / 展示面,非权威源**——D5 只读展示 Phase 现值 + 跳 H1 调整;D5 **owns** 的是非 Phase 参数(日限额 / 余额上限 / 网络费)。这一分工避免"两源"问题(H1 派发 + D5 又设一套)。

**② 后台界面**
两区:提现参数表 + Phase 联动现值。

1. **提现参数表**:五类参数现值 —— **日限额 / 余额上限 / 网络费(D5 owns,可配)** + **cooldown / NEX 闸(Phase 派发,只读展示)** + **complianceHoldEnabled(Phase 派发,只读)**:
   ...
   | NEX 闸(`withdrawNexGate`) | **H1 Phase 派发**(只读) | ❌ 只读,跳 H1 |
   | `complianceHoldEnabled` | **H1 Phase 派发**(只读) | ❌ 只读,跳 H1 |

2. **Phase 联动现值**:展示当前 Phase 下 cooldown / NEX 闸 / complianceHoldEnabled 的派发值(来自 H1)+ 距下一拐点(如"月 8:cooldown→35d;月 9:cooldown→45d + NEX 闸 10→20")+ "在 H1 调整 →"跳转。**非 Phase 参数**(日限 / 上限 / fee)提供手动覆盖配置入口。

**③ 可控参数**

> **默认值口径(§1.3:12 月运营周期优先,前端为现状参考,冲突以 12 月为准并注明差异)**:cooldown / NEX 闸以 **12 月 §6.4 为权威**(优先于前端 §13.4.1 简化实现);日限 / 上限 / fee 为前端 §9.3 / §9.11c.1 现状参考(12 月未覆盖)。
...
| NEX 闸(`withdrawNexGate`,**Phase 派发,只读**) | **月 1–8=10 / 月 9–12=20**(每 $100 燃烧 N NEX,12 月 §6.4) | 由 H1 Phase 引擎控制 | Phase 派发 | §6.4 `withdrawNexGate` / §9.3.2(每 $100 燃烧 10 NEX);**权威 H1**,D5 只读展示 |
...
> **cooldown / NEX 闸默认值忠实引用 12 月 §6.4**:cooldown **月 1–7=30d / 月 8=35d / 月 9+=45d**、NEX 闸 **月 1–8=10 / 月 9–12=20**,为 12 月 §6.4 月粒度权威目标值;其中月 8=35d 中间档须前端新增实现(前端 §13.4.1 现 Phase 实现为 P4=30d、P5 直接 45d,缺 35d 中间档)。**月 8=35d 为权威目标值**——本表正文行不否定 35d。**D5 不独立设 cooldown / NEX 闸**——其值由 H1 按 Phase 下发,D5 仅展示派发现值。
>
> **complianceHoldEnabled P5 带生效粒度**:P5 带(月 8)起即为 `true`,无月内拐点(§1.7 P5 整带月 8–10 均标 ✅);若实际实现出现月内拐点须显式标注。
>
> **dial 默认值单一来源声明(便于审计核查)**:本表 cooldown / NEX 闸默认值与 §1.7 8-dial 表严格对齐(P4 月 8=35d、P5 月 9+=45d / NEX 闸 月 9=20),与 12 月 §6.4 月粒度口径一致。
>
...
| 动作 | 角色 | 确认弹窗 | 审计点 |
|---|---|---|---|
| 非 Phase 参数配置(日限额 / 余额上限 / 网络费) | 财务(lead)/ 超管 | D5-MD1(理由必填+B1 红线预检(放大流出方向)) | `admin.withdraw_limit_changed`(field / before / after / operator / reason) |
| Phase 参数(cooldown / NEX 闸 / complianceHoldEnabled)展示 + 跳 H1 | 财务 / 风控 / 增长 | 否(只读展示 + 跳转;实际调整的确认弹窗在 H1) | 跳转 admin 审计事件(D5 仅记跳转,实际 dial 改动由 H1 记 `phase.dial_changed`) |

> **非 Phase 参数配置 = 高敏动作**(§1.8 原则二.4,2026-06 操作确认决议):日限额 / 余额上限 / 网络费直接调节资金流出摩擦,**执行权 = 财务(lead)/ 超管**(原复核层级转为执行门槛),经确认弹窗 D5-MD1 + 理由必填(server 强制非空 400 `REASON_REQUIRED`,8–200 字)即时生效,落 A2 审计并实时告警超管 / 财务 lead;**放大流出方向(上调日限额 / 上调余额上限 / 下调网络费)前置 B1 兑付覆盖率红线核验**(低于红线 server 拒绝 422 `COVERAGE_BELOW_REDLINE`),弹窗必有影响预览区;收紧方向不受此约束。
>
> **Phase 参数只读展示 + 跳 H1 调整(避免与 H1 双源)**:D5 **不独立设 cooldown / NEX 闸 / complianceHoldEnabled**——这三个是 Phase 派发 dial(权威 H1,§3.14),D5 只读展示 Phase 现值 + 提供"在 H1 调整 →"跳转;运营调这三项一律去 H1 调度器经 H1 的确认弹窗(理由必填)改 dial。**D5 owns 日限 / 上限 / fee** 三个非 Phase 参数的写入。这是"提现摩擦 = Phase 派发参数(H1)+ D 域固定参数(D5)"分工的落地。
...
| 动作(同④) | 触发控件 + 位置 | 形态 | 可用态规则 | 点击行为 |
|---|---|---|---|---|
| 非 Phase 参数配置 | ②1 提现参数表 `source:'d5'` 三行行内「修改」/ ②2 手动覆盖配置入口 | 行内按钮 / 次按钮 | 财务(lead)/超管渲染;Phase 派发三行不渲染此控件 | 打开弹窗 D5-MD1 |
| 跳 H1 调 Phase 参数 | ②1/②2 Phase 派发行「在 H1 调整 →」 | 链接 | 财务/风控/增长渲染 | 跳转 H1 调度器(留痕),无本地弹窗 |
| 查看参数 / Phase 现值 | ②1/②2 导航 | 链接 | 按角色裁剪 | 跳转视图,无弹窗 |

**(2) 弹窗规格**

##### [D5-MD1] 提现参数变更确认(日限额 / 余额上限 / 网络费)
- **功能**:变更 D5 owns 的非 Phase 提现参数(`dailyLimitCount` / `balanceMaxRatio` / `networkFeeRatio` + min/max),确认即时生效(网络费按 ③ 表"仅新对象",在途不变)。
- **布局结构**:1. **信息区**:本次变更字段(field / 当前值 → 目标新值)/ 方向判定标签(**放大流出** / 收紧流出)/ 当前 Phase 与 cooldown / points 派发现值(供联动研判,只读)。2. **影响预览区**(放大流出方向必有):server 预检「拟生效后兑付覆盖率」;低于黄线警示条(当前值 + 黄/红线);低于红线升级为阻断(确认钮置灰,文案含「覆盖率低于红线,server 将拒绝(422)」);收紧方向展示提示行「收紧流出,不受红线约束,正常生效」。3. **输入区**:目标新值控件(日限额 = 数字输入 1–10 次/日;余额上限 = 数字输入 50%–100%;网络费 = 比例 0%–5% + min/max 数字输入;范围引 ③ 表)+ reason(多行文本,必填,8–200 字;server 空值 400 `REASON_REQUIRED`)。4. **按钮区**:`[取消]` · `[确认变更]`(必填未过 / 超 ③ 表范围 / 覆盖率阻断置灰;loading 防双击)。
...
**⑤ 接口**
- `GET /api/admin/withdraw/limits` — 返回提现参数现值 `{ dailyLimitCount, balanceMaxRatio, networkFeeRatio, networkFeeMin, networkFeeMax, cooldownDays(来自 H1 Phase 派发,只读,source:'phase-h1'), nexGatePer100(来自 H1,只读,source:'phase-h1'), complianceHoldEnabled(来自 H1,只读,source:'phase-h1'), currentPhase }`;日限 / 上限 / fee 标注 `source:'d5'`(可写)。对齐前端 §9.11c.1 `GET /api/config/wallet/withdraw-limits`。
- `PUT /api/admin/withdraw/limits`(**仅非 Phase 参数**:`dailyLimitCount / balanceMaxRatio / networkFeeRatio / networkFeeMin / networkFeeMax`,确认弹窗 D5-MD1:body 必携 `{reason}`,空值 400 `REASON_REQUIRED`,确认即生效;**请求体含 cooldownDays / nexGatePer100 / complianceHoldEnabled 时 server 返回 422 Unprocessable Entity** + error body `{code:'PHASE_PARAM_READONLY', message:'cooldownDays/nexGatePer100/complianceHoldEnabled 为 Phase 派发参数，请经 H1 调整', redirect:'/admin/phase/h1'}` —— 这些参数只能经 H1 `PUT /api/admin/phase/dials` 修改,D5 不接受其写入)。

> **D5 PUT 拒绝 Phase 参数写入(防双源)**:`PUT /api/admin/withdraw/limits` 服务端**仅接受非 Phase 参数**;若请求体携带 `cooldownDays` / `nexGatePer100` / `complianceHoldEnabled`,server 返回 **422 Unprocessable Entity** + error body `{code:'PHASE_PARAM_READONLY', message:'cooldownDays/nexGatePer100/complianceHoldEnabled 为 Phase 派发参数，请经 H1 调整', redirect:'/admin/phase/h1'}`。这从接口层 enforce "D5 不是这些参数的权威源"(§3.14)。
>
> **PUT 放大流出方向为覆盖率红线强约束(§1.8 原则一)**:server 在执行确认时,若操作朝**放大流出**方向(上调 `dailyLimitCount`、上调 `balanceMaxRatio`、下调 `networkFeeRatio`),且 B1 兑付覆盖率**低于红线(100%)**,server **拒绝**(返回 422 `COVERAGE_BELOW_REDLINE` + 当前覆盖率值;拒绝码全卷统一 422,PM 2026-06-02),而非仅附快照建议;**收紧方向**(降日限、降余额上限、升网络费)**不受此约束**,正常生效。这与 §1.8 原则一(放大流出前置强约束)一致;弹窗影响预览区的「拟生效后兑付覆盖率」预检与本约束同口径(D5-MD1)。
...
| 查看提现参数 / Phase 现值 | ✅ | ✅ | ✅ | ✅(只读) | — | ✅ |
| 非 Phase 参数配置 | ✅ | ✅(lead) | — | — | — | — |
| 跳 H1 调 cooldown / NEX 闸 / complianceHold | ✅ | ✅ | ✅ | ✅ | — | — |

审计记录字段:`操作者 / 角色 / 动作(limit_change|jumpToH1) / field / 前值 / 后值 / 原因 / 时间(ms)`;cooldown / NEX 闸 / complianceHoldEnabled 的实际变更审计由 H1 记录(`phase.dial_changed` + A2 admin 审计,§2.4.5 ⑥)。

**⑦ 风控 & 联动**
- **cooldown / NEX 闸 / complianceHoldEnabled 由 Phase 引擎(H1)按用户 Phase 下发,D5 是生效面非权威源**(§3.14 铁律):这三个参数的权威值在 H1,按用户 `joinedAt` → 所处 Phase 派发(§1.7 月粒度);D5 展示派发现值并作 D2 队列"NEX 是否足烧 / 冷却是否到期 / 是否进增强合规审查"判定的生效面,**不重设、不缓存为权威**,避免 H1 派发 + D5 又设一套的双源分叉。
- **D5 owns 日限 / 上限 / fee**:日限额、余额上限、网络费三个非 Phase 参数由 D5 单一拥有与配置(server-canonical);其调整属放大 / 收紧资金流出,**放大流出方向改前须过 B1 覆盖率红线强约束**(§1.8 原则一:覆盖率低于红线时 server 拒绝放大流出,见 ⑤);收紧方向不受此约束。
- **联动 D2 队列**:D5 生效的五类参数 + complianceHoldEnabled 是 D2 提现队列的判定输入——日限(24h 第几笔)、余额上限(可提额)、网络费(到账额)、cooldown(冷却到期)、NEX 闸(每 $100 燃烧 N NEX)、complianceHold(P5+ 增强审查);D2 消费 D5 生效值,不重设。
- **Phase 拐点连带**:H1 在 Phase 拐点改 cooldown / NEX 闸 / complianceHoldEnabled(如月 8→35d P4 带末月粒度拐点、月 9→45d + NEX 闸 20、P5 带月 8→complianceHold true)时,D5 Phase 联动现值同步更新展示;运营在拐点须知 D5 展示随 H1 派发变化(非 D5 自行改动)。

**⑧ 埋点(事件)**
对齐 A4(Ch2 §2.4),D5 **消费** Phase dial 变更、**产生** 非 Phase 参数审计事件:

- **消费(§2.4.5 ⑥ phase / admin)**:`phase.dial_changed`(H1 改 cooldown / NEX 闸 / complianceHoldEnabled dial → D5 联动展示现值变化,§3.14 权威归 H1);并按 `phase` 属性感知当前用户群的派发现值。
- **产生**:`admin.withdraw_limit_changed` — 触发点:非 Phase 参数(日限 / 上限 / fee)配置确认执行;属性:`field(dailyLimit|balanceMax|networkFee) / before / after / operator / reason`;喂 L3 财务报表(提现摩擦运营记录)。**注**:cooldown / NEX 闸 / complianceHoldEnabled 的变更事件 `phase.dial_changed` **由 H1 产生**(§3.14),D5 不产生。
...
>
> **跨域权威声明**:**H1 是全平台 Phase 模型(后台管理共 8 项 dial)的唯一权威操作源与值定义源**。第 1 章 §1.7 是 phase-band 汇总速查表(admin PRD §1.7 已称「8-dial 表」),**本章 §H1 的逐月矩阵是逐值权威**,两者必须逐值一致;若不一致以本章为准并同步修订 §1.7。下游消费方均为只读引用:B4(第 4 章节奏状态,只读展示)、D5(第 6 章提现 cooldown/withdrawNexGate 生效面)、F3(双轨封顶,V2)。
>
> **dial 计数口径(一次性说明)**:本后台文档的权威模型为 **后台管理共 8 项 dial**,= 12 月节奏表 §6.4 `ProductPhase` 的 7 个时变 dial(新用户加成 `newUserBonusMultiplier` / 邀请加成 `inviteRewardMultiplier` / 复投加成 `reinvestMultiplier` / 提现 NEX 闸 `withdrawNexGate` / 提现冷却 `withdrawCooldownDays` / 双轨日封顶 `binaryDailyCap` / 任务加成 `questBonusMultiplier`)+ 1 个叠加 dial(`complianceHoldEnabled`,前端 §13.4.1 新增,§6.4 未覆盖)。**Premium 可用 / NEXv2 锁仓可用两 gate dial 随产品下线移除**。即:**7-dial(§6.4 ProductPhase 接口,去 Premium/NEXv2 gate)+ 1 叠加 dial(complianceHoldEnabled,§13.4.1)= 8 项**,与 §1.7 / §3.14 / B4 已写「8 dial」口径一致。
>
...
平台按 12 个运营月推进,划分为 6 个 Phase 段(P1–P6),每个运营月对应一组 dial 取值(后台管理共 8 项)。这 8 项 dial 共同决定全站的拉新加成、复投激励、资金流出闸门(提现冷却 / NEX 闸)、双轨封顶、任务加成与合规留存强度。本模块的职责是:让运营在单一受控界面查看、推进、覆盖 Phase,并在改动生效前预览全站下游影响。
...
| 超级管理员(Super Admin,对应 §1.1 RBAC 角色) | **执行全部 dial 改动**——含放大资金流出方向的 dial 改动(降 `withdrawCooldownDays` / 降 `withdrawNexGate` / 升 `binaryDailyCap`,**执行权仅超管**,server 前置 B1 兑付覆盖率红线核验)及 pin/override/定时切换配置;跨域仲裁;查看审计轨迹 |
...
**(d) 沙盒预览**
- 选定一组 dial 改动后,预览其对全站下游的影响(D5 提现 cooldown/withdrawNexGate 面、F3 双轨封顶、受影响用户规模、预估资金流出方向变化)。
...
| 运营月 | newUserBonusMultiplier 新用户加成 | inviteRewardMultiplier 邀请加成 | reinvestMultiplier 复投加成 | withdrawNexGate 提现 NEX 闸 | withdrawCooldownDays 提现冷却(天) | binaryDailyCap 双轨日封顶 | questBonusMultiplier 任务加成 | complianceHoldEnabled 合规留存 |
|---|---|---|---|---|---|---|---|---|
| 月 1 | 2 | 2 | 1 | 10 | 30 | 5000 | 4 | 否 |
| 月 2 | 2 | 2 | 1 | 10 | 30 | 5000 | 4 | 否 |
| 月 3 | 1.5 | 1.5 | 1 | 10 | 30 | 5000 | 1 | 否 |
| 月 4 | 1.5 | 1.5 | 1 | 10 | 30 | 5000 | 1 | 否 |
| 月 5 | 1 | 1 | 2 | 10 | 30 | 5000 | 1 | 否 |
| 月 6 | 1 | 1 | 2 | 10 | 30 | 5000 | 1 | 否 |
| 月 7 | 1 | 1 | 1 | 10 | 30 | 2000 | 1 | 否 |
| 月 8 | 1 | 1 | 1 | 10 | 35 | 2000 | 1 | 是 |
| 月 9 | 1 | 1 | 1 | 20 | 45 | 2000 | 1 | 是 |
| 月 10 | 1 | 1 | 1 | 20 | 45 | 2000 | 1 | 是 |
| 月 11 | 1 | 1 | 1 | 20 | 45 | 2000 | 1 | 是 |
| 月 12 | 1 | 1 | 1 | 20 | 45 | 2000 | 1 | 是 |
...
| withdrawNexGate 提现 NEX 闸 | 月 1-8 = 10;月 9-12 = 20 | 0–100(每 $100 燃烧 N NEX) | 实时全量(D5 提现生效面) | 同上 |
...
| 动作(同⑤) | 触发控件 + 位置 | 形态 | 可用态规则 | 点击行为 |
|---|---|---|---|---|
| 改 dial 值 | ③(b)逐月矩阵单元格下钻「改值」+ ③(d)沙盒预览「转为正式改动」 | 行内按钮 / 次按钮 | 增长/超管渲染;放大流出方向 dial(降 cooldown / 降 NEX 闸 / 升 cap)仅超管可提交(增长可打开预览,确认钮置灰并提示执行权) | 打开弹窗 H1-MD1 |
...
- **布局结构**:
  ...
  2. **影响预览区**(必有):「该 dial 当前值 → 目标值」并排;**影响 cohort 数与命中用户规模**(含被 override/pin 命中人群拆分);**方向判定(放大流出 / 收紧 / 中性)**——放大流出方向(降 `withdrawCooldownDays` / 降 `withdrawNexGate` / 升 `binaryDailyCap`)必展示 server 预检「拟生效后兑付覆盖率」,低于黄线警示、低于红线升级为**阻断**(确认钮置灰,文案含「覆盖率低于红线,server 将拒绝(422)」);下游联动摘要(D5 / F3,由沙盒推演派生)。
  3. **输入区**:见下表。
  4. **按钮区**:取消 / 确认改动。
- **输入与选择控件**:
...
- **布局结构**:1. **信息区**:目标 cohort(`YYYY-Www`)/ 该 cohort 当前有效月与命中用户规模快照。2. **影响预览区**:偏移后有效月 → 该月 8-dial 值与当前值的逐项 diff;方向判定——偏移使提现参数较当前**放宽**(等效放大流出,如延迟至低 NEX 闸/短冷却月份)时,展示 server 预检「拟生效后兑付覆盖率」,低于红线阻断。3. **输入区**:见下表。4. **按钮区**:取消 / 确认生效(置灰条件 / loading;携 `Idempotency-Key`)。
- **输入与选择控件**:
...
1. **改 dial 值**(高敏 · 确认弹窗 H1-MD1)
   ...
   - **流出方向红线前置 + 执行权就高**:若改动为放大资金流出方向(降 `withdrawCooldownDays` / 降 `withdrawNexGate` / 升 `binaryDailyCap`),**执行权仅超管**(2026-06 操作确认决议),且 server 强制前置 B1 兑付覆盖率红线核验——**覆盖率低于 B1 `coverageRedLine`(默认 100%,§[B1] ③)时 server 拒绝写入(422 `COVERAGE_BELOW_REDLINE`)**,弹窗内强制展示覆盖率预检;非放大流出方向执行 = 增长 / 超管。
...
- **改 dial 联动下游**:任一 dial 改动须同步反映到下游生效面——`withdrawCooldownDays` / `withdrawNexGate` → D5 提现;`binaryDailyCap` → F3 双轨封顶;`complianceHoldEnabled` → D5/D2 增强合规审查。沙盒预览须覆盖这些联动,正式生效须触发下游一致刷新。
...
>
> **默认值依据**:大额 $1,000 锚定前端 §9.3.5 合规 hold「over $1,000」,与 12 月运营周期 P5–P6 提现摩擦放大方向一致;速度 / 新账户保护期为 12 月周期未覆盖项,按提现套现防御逻辑设默认。冷却 / NEX 闸门槛**不在 K3 设定**——其为 Phase 派发参数,权威归 H1、生效面在 D5(§3.14),K3 只产出路由结论不重设资金门槛。
...
- **server-side enforce,client 不可绕**(§9.11d.2):提现请求的规则评估在服务端,客户端无法跳过;提现 endpoint 对 KYC 地址匹配(§16.2)、NEX 闸门槛(C4/D5 生效面)二次 enforce,K3 规则路由叠加其上。
...
| **Withdrawal 扩展态**(提现状态机) | 状态枚举 = **正常 5 态**(`submitted / review-passed / processing / sent / confirmed`,§9.3.6)+ **异常 6 态**(`review-rejected / address-invalid / tx-failed / tx-orphaned / refunded / frozen`,§9.11f)+ **后台扩展中间态 `review-pending`**(D2② 状态机已实现,V1 落地);字段 `withdrawalId(server mint)/ userId / amountUsdt / address(hash)+chain / riskScore(K4)/ kycStatus(C4)/ nexGateOk / count24h / hitRules(K3)/ state` | Ch6 D2(§9.3 / §9.3.6 / §9.11f) | **`Withdrawal` 实体本体属用户侧(§12,本表不重列)**;本行仅登记 D2 在前端 §9.3.6 正常 5 态之上**新增的异常 6 态(§9.11f)**及**后台扩展中间态 `review-pending`**。`review-pending` 为 **V1 已实现的后台扩展中间态**(D2② 状态机图 `submitted ─风控评分─▶ review-pending`;`/api/admin/withdrawals` status 枚举 已列),承接 `delay` / `freeze` / `reject` / `approve` 四条转出边。V1 落地状态为 **后台扩展态 review-pending + 正常 5 态 + 异常 6 态共 12 态**;**§1.9 术语表当前仅定义 11 态(正常 5 + 异常 6),`review-pending` 补入 §1.9**(D2 已声明此偏差)。全 server-canonical,client 仅订阅;状态推进非法转移返回 409。`nexGateOk` = 用户 NEX 余额是否足以覆盖提现燃烧量(`withdrawNexGate`,Phase 派发,D5 生效面)|
...
| D5 | `/api/admin/withdraw/limits` | GET | 提现参数现值(日限/上限/fee 可写 `source:'d5'`;cooldown/NEX 闸/complianceHold 只读 `source:'phase-h1'`) | Ch6 D5 | — | — |
