...
| AC-C6-02 | 注册预验与认证恢复 | 错误/过期/跨手机号或环境验证码保持验码页；正确预验才进入密码且不消费、不建号或会话；最终提交重验消费。编辑输入、重发或离页后的旧登录/验码响应不推进步骤、不锁死按钮 | PK-PROTO、PK-IDEM、PK-CROSS |
| AC-C5-02 | 密码修改与重置 | 忘记密码先预验再设新密码，成功吊销会话并回密码登录；账户内改密只提交等待前已校验输入，原成功回执只恢复不重复执行 | PK-IDEM、PK-CROSS |
...
| AC-D2-02 | 银行提现与原单恢复 | 首次绑卡免 OTP、换卡专属短信且立即生效 → 锁定报价 → 当前唯一 NOT_SUBMITTED 提交 → 多端/未知结果恢复 → D2 查原单；账户编号/版本变化、用途串码和资金证据不一致均被拒绝，只有原单与账本证据匹配才展示到账/退款 | PK-IDEM、PK-CAS、PK-CROSS、PK-PERM |
...
| AC-D7-01 | 银行法币提现配置 | 读取参数与真实服务商就绪态 → 测版本冲突、越界、权限不足、开启请求及关闭时原单恢复 → 服务商或覆盖率不满足则拒绝开启；不要求已撤销的外部账户核验字段；配置写入不产生到账事实 | PK-PERM、PK-CAS、PK-IDEM、PK-CROSS |

银行正向资金测试只使用授权隔离数据和模拟供应商；不修改真实代付开关，不把这些用例通过当作真实银行到账验收。真实供应商契约及授权出款验收未完成前保持 HOLD。当前 App 仍待适配换卡短信与已撤销的外部核验门，后端/PC 修改不代表三端闭环已通过。
