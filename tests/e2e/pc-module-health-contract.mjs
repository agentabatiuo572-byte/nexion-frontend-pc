...
const allowedOperationalAlerts = {
  ...
  F4: [
    /^提前结算已暂停\s*·\s*领导奖池结算配置不可用：(?:配置版本(?:、奖池比例)?(?:、月度\s+cap)?(?:、解锁等级)?(?:、结算周期)?|奖池比例(?:、月度\s+cap)?(?:、解锁等级)?(?:、结算周期)?|月度\s+cap(?:、解锁等级)?(?:、结算周期)?|解锁等级(?:、结算周期)?|结算周期)\s*缺失或格式错误。配置入口就在本卡片下方，请依次补齐后重新读取配置。重新读取配置$/i,
  ],
};
