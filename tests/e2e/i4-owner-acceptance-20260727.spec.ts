...
const EVIDENCE_DIR =
  process.env.I4_EVIDENCE_DIR ||
  "D:/workspace/nexion-ops-console/docs/验收报告/PC全面测试-20260726/evidence/I4-owner";

test.beforeAll(async () => {
  await mkdir(EVIDENCE_DIR, { recursive: true });
});

test("首次用户从登录和可见侧栏进入 I4，刷新和重登后恢复六版块服务端事实", async ({ page }) => {
...
test("I4 权威读取失败时不展示伪版块，恢复接口后可重新读取", async ({ page }) => {
  await login(page);
  await page.route("**/api/admin/content/trust-disclosure/overview", (route) =>
    route.fulfill({
      status: 503,
      contentType: "application/json",
      body: JSON.stringify({ code: 503, message: "I4_TEST_UNAVAILABLE", data: null }),
    }),
  );
  await openI4(page);
...
async function openI4(page: Page) {
  ...
  const link = page.locator('a[href="/content/trust"]').first();
  await expect(link).toBeVisible({ timeout: 10_000 });
  await link.click();
  await expect(page).toHaveURL(/\/content\/trust(?:\?.*)?$/, { timeout: 20_000 });
}

async function expectI4Ready(page: Page) {
  await expect(page.getByText("受管信任版块", { exact: true })).toBeVisible({ timeout: 20_000 });
  await expect(page.getByText("6 个", { exact: true })).toBeVisible();
  await expect(page.getByRole("cell", { name: "financials", exact: true }).first()).toBeVisible();
  await expect(page.getByRole("cell", { name: "leadership", exact: true }).first()).toBeVisible();
  await expect(page.getByText("I4 暂无真实接口数据")).toHaveCount(0);
}
