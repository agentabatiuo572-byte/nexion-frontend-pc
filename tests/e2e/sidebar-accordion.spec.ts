...
test.beforeEach(async ({ page }) => {
  ...
  await page.addInitScript(() => {
    if (window.localStorage.getItem("nexion-sidebar-v2-seeded") === "1") return;
    ...
    window.localStorage.setItem("nexion-sidebar-v2-seeded", "1");
  });
});
...
test("sidebar starts collapsed and keeps only the selected group open", async ({ page }) => {
  ...
  expect(persistedUi.version).toBe(3);
