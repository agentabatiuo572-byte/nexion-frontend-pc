...
test("L 域权限夹具精确清理并由独立 checker 批准角色删除", async ({ page, browser }) => {
  ...
  try {
    ...
    for (const role of fixture.roles) {
      ...
      const current = overview.roles.find((candidate) =>
        candidate.id === role.id || candidate.roleCode === role.roleCode);
      if (!current) continue;
      const proposal = await okEnvelope<Record<string, unknown>>(
        await page.request.delete(`/api/admin/platform/roles/${current.id}`, {
          headers: { "Idempotency-Key": `${RUN_ID}-cleanup-l-role-${current.id}` },
          data: {
            reason: `${RUN_ID} L 域验收复审完成后删除隔离角色`,
      ...
      await okEnvelope(await checkerPage.request.post(
        ...
        {
          headers: { "Idempotency-Key": `${RUN_ID}-cleanup-l-role-${current.id}-approve` },
          data: { reason: `${RUN_ID} 独立复核删除 L 域隔离角色` },
        },
      ));
    }
  } finally {
...
async function mutateAccount(
  ...
  await okEnvelope(await page.request.fetch(`/api/admin/platform/accounts/${accountId}/${suffix}`, {
    method,
    headers: { "Idempotency-Key": `${RUN_ID}-cleanup-l-${accountId}-${suffix}` },
    data: {
      ...data,
      operator: ROOT_USERNAME,
...
async function accountById(page: Page, accountId: string) {
  ...
  expect(account, `L fixture account ${accountId} must exist`).toBeTruthy();
  return account!;
}
