...
test.describe.serial("K2 套利与刷量检测独立验收", () => {
  ...
  test("具体动作权限、幂等重放与陈旧版本CAS均由服务器约束", async ({ page, browser }) => {
    await login(page);
    ...
    let roleId = "";
    try {
      const checker = await createChecker(page, checkerPage, "grant");
      checkerId = checker.accountId;
      const roleCode = `K2_FLAG_${SUFFIX}`;
      const role = await api(page, "POST", "/api/admin/platform/roles", { roleCode, roleName: `K2标记验收${SUFFIX}`, remark: RUN_ID, status: 1, reason: `${RUN_ID} 创建K2精确权限角色`, operator: "superadmin" });
      expect(role.status, role.raw).toBeLessThan(400);
      roleId = String(role.data.id);
      const menus = await api(page, "GET", "/api/admin/platform/menus/overview");
      const flat: any[] = [];
      const visit = (nodes: any[]) => nodes.forEach((item) => { flat.push(item.node); visit(item.children || []); });
      visit(menus.data.tree || []);
      const menuIds = flat.filter((item) => item.menuCode === "K" || item.menuCode === "K2").map((item) => item.id);
      expect(menuIds).toHaveLength(2);
      const grant = await api(page, "PUT", `/api/admin/platform/roles/${roleId}/grants`, { permissionCodes: ["risk_k2_read", "risk_k2_row_flag"], menuIds, reason: `${RUN_ID} 绑定K2只标记权限`, operator: "superadmin" });
      expect(grant.status, grant.raw).toBeLessThan(400);
      const ticketId = String(grant.data.operationId ?? grant.data.id);
      const approved = await api(checkerPage, "POST", `/api/admin/platform/audit/operations/${ticketId}/approve`, { reason: `${RUN_ID} 独立复核K2精确授权`, operator: checker.username });
      expect(approved.status, approved.raw).toBeLessThan(400);

      const readerUsername = `k2r${SUFFIX}`;
      const initialPassword = `K2Reviewer@${SUFFIX}aA!`;
      const readerPassword = `K2Reviewer@${SUFFIX}bB!`;
      ...
      try {
        ...
        expect(await readerPage.getByRole("button", { name: "标记套利", exact: true }).count())
          .toBeGreaterThan(0);
        await expect(readerPage.getByRole("button", { name: /拦截新人礼|标记刷榜|联动 K1 冻结/ })).toHaveCount(0);
        const commandKey = `k2-perm-${randomUUID()}`;
        const body = { expectedVersion: 0, reason: `${RUN_ID} 精确标记权限幂等验证` };
        const first = await api(readerPage, "POST", `/api/admin/risk/arbitrage/rows/${ROWS.permission}/mark`, body, commandKey);
        const replay = await api(readerPage, "POST", `/api/admin/risk/arbitrage/rows/${ROWS.permission}/mark`, body, commandKey);
        const stale = await api(readerPage, "POST", `/api/admin/risk/arbitrage/rows/${ROWS.permission}/mark`, body);
        expect(first.status, first.raw).toBe(200);
        expect(replay.status, replay.raw).toBe(200);
        expect(stale.status, stale.raw).toBe(409);
        const forbiddenGift = await api(readerPage, "POST", `/api/admin/risk/arbitrage/rows/${ROWS.gift}/block-gift`, { expectedVersion: 0, reason: `${RUN_ID} 越权探针不得执行` });
        const forbiddenParam = await api(readerPage, "PATCH", "/api/admin/risk/arbitrage/params/otpGate.resendSeconds", { value: "61", expectedVersion: 0, reason: `${RUN_ID} 越权参数探针不得执行` });
        expect(forbiddenGift.status, forbiddenGift.raw).toBe(403);
        expect(forbiddenParam.status, forbiddenParam.raw).toBe(403);
        await shot(readerPage, "02-exact-permission");
      } finally { await readerContext.close(); }
      await cleanupAccount(page, readerId, `${RUN_ID} 清理K2精确权限账号`); readerId = "";
      const removed = await api(page, "DELETE", `/api/admin/platform/roles/${roleId}`, { reason: `${RUN_ID} 删除K2精确权限角色`, operator: "superadmin" });
      const deleteTicket = String(removed.data.operationId ?? removed.data.id);
      const deleteApproved = await api(checkerPage, "POST", `/api/admin/platform/audit/operations/${deleteTicket}/approve`, { reason: `${RUN_ID} 独立复核删除K2角色`, operator: checker.username });
  ...
  test("OTP参数可逆写入、刷新重登、幂等重放、CAS和审计后恢复", async ({ page }) => {
    ...
    try {
      ...
      await page.getByRole("button", { name: /K2 Independent Admin|Super Admin/ }).first().click();
