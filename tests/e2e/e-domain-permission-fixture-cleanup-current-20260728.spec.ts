...
const E6_MAKER_FIXTURE_PATH = process.env.E6_MAKER_FIXTURE_PATH
...
const EVIDENCE_DIR = process.env.E_PERMISSION_CLEANUP_EVIDENCE_DIR
  ?? `D:/workspace/bug-pic/.restricted/${RUN_ID}/E/nonowner-D/permission-cleanup`;

type ApiEnvelope<T> = { code?: number; message?: string; data?: T };
type Account = { accountId: string; username: string; password: string };
type Fixture = {
  accounts: Record<string, Account>;
  customRole: { id: number; roleCode: string };
};
type E6MakerFixture = {
  account: Account;
  customRole: { id: number; roleCode: string };
...
test("E 域权限夹具按最新 CAS 精确清理，并由独立 checker 删除隔离角色", async ({ page, browser }) => {
  expect(ROOT_PASSWORD, "ADMIN_E2E_PASSWORD is required").not.toBe("");
  mkdirSync(EVIDENCE_DIR, { recursive: true });
  const fixture = JSON.parse(readFileSync(FIXTURE_PATH, "utf8")) as Fixture;
  const e6MakerFixture = JSON.parse(readFileSync(E6_MAKER_FIXTURE_PATH, "utf8")) as E6MakerFixture;
  const checker = (JSON.parse(readFileSync(CHECKER_FIXTURE_PATH, "utf8")) as CheckerFixture)
    .accounts.d_checker;
  const allAccounts = [...Object.values(fixture.accounts), e6MakerFixture.account];
  const allRoles = [fixture.customRole, e6MakerFixture.customRole];
  ...
  const roleDeleteTickets: Array<{ role: { id: number; roleCode: string }; ticketId: string }> = [];
  ...
  try {
    ...
    for (const isolatedRole of allRoles) {
      ...
      const currentRole = overview.roles.find((candidate) =>
        candidate.id === isolatedRole.id
        || candidate.roleCode === isolatedRole.roleCode);
      if (currentRole) {
        const proposal = await okEnvelope<Record<string, unknown>>(
          await page.request.delete(`/api/admin/platform/roles/${currentRole.id}`, {
            headers: { "Idempotency-Key": `${RUN_ID}-cleanup-e-role-${currentRole.id}` },
            data: {
              reason: `${RUN_ID} E 域非 Owner 复审完成后删除隔离角色`,
        ...
        await okEnvelope(await checkerPage.request.post(
          ...
          {
            headers: { "Idempotency-Key": `${RUN_ID}-cleanup-e-role-${currentRole.id}-approve` },
            data: { reason: `${RUN_ID} 独立复核删除 E 域隔离角色` },
          },
        ));
        roleDeleteTickets.push({ role: isolatedRole, ticketId });
      }
    }
  } finally {
  ...
  const finalAccounts = allAccounts.map((account) => {
  ...
  const finalRoles = await okEnvelope<{ roles: Array<{ id: number; roleCode: string }> }>(
    await page.request.get("/api/admin/platform/roles/overview"),
  );
  for (const isolatedRole of allRoles) {
    expect(finalRoles.roles.some((candidate) =>
      candidate.id === isolatedRole.id
      || candidate.roleCode === isolatedRole.roleCode)).toBe(false);
  }
  ...
  expect(audit.operationQueue.some((ticket) =>
    ...
    && allRoles.some((role) => ticket.obj.includes(role.roleCode)))).toBe(false);
...
async function loginChecker(
  ...
  // J-AUTO-003: discard a failed/replayed challenge and retry only with a
  // strictly newer TOTP counter after a fresh password login.
...
async function mutateAccount(
  ...
  await okEnvelope(await page.request.fetch(`/api/admin/platform/accounts/${accountId}/${suffix}`, {
    method,
    headers: { "Idempotency-Key": `${RUN_ID}-cleanup-e-${accountId}-${suffix}` },
    data: {
      ...data,
      operator: ROOT_USERNAME,
...
async function accountById(page: Page, accountId: string) {
  ...
  expect(account, `E fixture account ${accountId} must exist`).toBeTruthy();
  return account!;
}
