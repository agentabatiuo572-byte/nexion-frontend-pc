...
const FINAL_PASSWORD = process.env.M_PERMISSION_FINAL_PASSWORD ?? "";
const RUN_ID = process.env.M_PERMISSION_RUN_ID ?? "pc-full-acceptance-20260728-151023";
const FIXTURE_PATH = process.env.M_PERMISSION_FIXTURE_PATH
  ?? `D:/workspace/bug-pic/.restricted/${RUN_ID}/M/permission-fixtures.json`;
const SUFFIX = process.env.M_PERMISSION_SUFFIX ?? Date.now().toString(36);

type AccountKey = "m_readonly" | "m_no_write" | "m_no_menu";
type FixtureAccount = {
  id: string;
  username: string;
  password: string;
...
test("创建并激活 M 域 readonly/no-write/no-menu 独立权限夹具", async ({ page }) => {
  expect(ROOT_PASSWORD, "ADMIN_E2E_PASSWORD is required").not.toBe("");
  expect(FINAL_PASSWORD, "M_PERMISSION_FINAL_PASSWORD is required").not.toBe("");
  ...
  const definitions: Array<{ key: AccountKey; role: FixtureAccount["role"] }> = [
    { key: "m_readonly", role: "auditor" },
    { key: "m_no_write", role: "auditor" },
    { key: "m_no_menu", role: "finance" },
  ];
...
async function assertExpectedSession(page: Page, key: AccountKey) {
  const data = await okEnvelope<{ session?: { authorities?: string[]; menuCodes?: string[]; effectiveMenus?: unknown[] } }>(
    await page.request.get("/api/admin/auth/session"),
  );
  const authorities = data.session?.authorities ?? [];
  if (key === "m_no_menu") {
    expect(authorities.some((authority) => authority.startsWith("service_m"))).toBe(false);
  } else {
